...
# --- V81 - "MARIO BATTLE HYPER-STABLE (NETWORK FIX)" ---
# Technical Specs:
# - NETWORK THROTTLING: Sync updates reduced from 60Hz to 5Hz to prevent memory leaks/freezes.
# - ASYNC TASK MANAGEMENT: Uses gated tasks to ensure only one sync happens at a time.
...
BS = 32
GRID_Y = 60
SYNC_INTERVAL = 0.25 # 4 times per second (prevents freezing)
POLL_INTERVAL = 0.50 # 2 times per second
...
class BattleNetwork:
    def __init__(self, db_url):
        ...
        self.room_id = "battle_v81_stable"
    ...
    async def poll_state(self):
        ...
        if now - self.last_poll > POLL_INTERVAL:
            ...
            if data:
                ...
                if opp_data.get('lost') and not self.game_over: self.game_over = True
    ...
    async def push_sync(self, grid, score, stomps):
        now = time.time()
        if self.sync_lock or (now - self.last_sync_time < SYNC_INTERVAL):
            return
...
class ProfessionalEnemy:
    ...
    def update(self, dt, grid, audio):
        ...
        if self.state == 'walking':
            ...
            elif 0 <= int(self.y + 0.5) < 20 and grid[int(self.y + 0.5)][nx_idx] is not None: turn = True
            if turn: self.dir *= -1
            else: self.x = next_x
...
class MarioBattlePro:
    def __init__(self):
        ...
        pygame.display.set_caption("MARIO BATTLE STABLE V81")
    ...
    def pull_piece(self):
        ...
        t = self.bag.pop()
        map_p = {'I':[(0,0),(1,0),(2,0),(3,0)], 'O':[(0,0),(1,0),(0,1),(1,1)], 'T':[(1,0),(0,1),(1,1),(2,1)], 'S':[(1,0),(2,0),(0,1),(1,1)], 'Z':[(0,0),(1,0),(1,1),(2,1)], 'J':[(1,0),(1,1),(1,2),(0,2)], 'L':[(1,0),(1,1),(1,2),(2,2)]}
                ...
                if e.type == pygame.KEYDOWN:
                    if self.phase == "menu":
                        ...
                        if e.key == pygame.K_SPACE:
                            while not self.hit_test([self.piece_pos[0], self.piece_pos[1]+1], self.piece['shape']): self.piece_pos[1] += 1
                            self.audio.play('lock') ; self.fall_count = 10.0
                    ...
                    if not self.net.game_over:
                        if self.clearing_rows:
                            ...
                            for bx, by in self.piece['shape']:
                                ...
                                if 0 <= gx < 10 and 0 <= gy < 20: shadow[gy][gx] = c_col
                            sync_str = "".join([COLOR_TO_ID.get(c, "0") if c != (140,140,140) else "G" for row in shadow for c in row])
                            ...
                            if self.net.incoming_attack > 0:
                                ...
                                self.comms.append(CombatText("⚠️ ATTACK RECV!", (255, 50, 50), 320, 220, self.f_m))
                                ...
                                if en.update(dt, self.grid, self.audio): self.enemies.remove(en)
                                    ...
                                    for bx, by in self.piece['shape']:
                                        if int(en.x + 0.5) == self.piece_pos[0]+bx and int(en.y + 0.5) == self.piece_pos[1]+by:
                                            ...
                                            self.comms.append(CombatText("STOMP HI! 🚀", (0,255,255), 320, 260, self.f_m))
                            ...
                            if self.fall_count > fall_spd:
                                ...
                                if self.hit_test(self.piece_pos, self.piece['shape']):
                                    ...
                                    if self.clearing_rows:
                                        ...
                                        self.banners.append(AchievementBanner(f"COMBO: {len(self.clearing_rows)}!", (0,255,255), self.f_m))
                                        asyncio.create_task(self.net.commit_attack(len(self.clearing_rows)))
                                    else:
                                        self.piece_pos = [GW//2-1, 0] ; self.piece = self.pull_piece()
                                        if self.hit_test(self.piece_pos, self.piece['shape']): await self.net.report_defeat()
            self.draw_frame() ; await asyncio.sleep(0.01)
    ...
    def draw_frame(self):
        ...
        if self.phase == "menu":
            ...
            self.draw_t("FIXED VERSION v81", self.f_m, (255,255,0), WW//2, 350)
            ...
            if self.phase == "play" and not self.clearing_rows and not self.net.game_over:
                ...
                col = ID_TO_COLOR[{'I':'1','O':'2','T':'3','S':'4','Z':'5','J':'6','L':'7'}[self.piece['id']]]
                for bx, by in self.piece['shape']:
                    gst = pygame.Surface((BS, BS), pygame.SRCALPHA) ; gst.fill((*col, 75))
            ...
            if not self.clearing_rows and self.phase == "play" and not self.net.game_over:
                cid = self.piece['id'] ; scol = ID_TO_COLOR[{'I':'1','O':'2','T':'3','S':'4','Z':'5','J':'6','L':'7'}[cid]]
            ...
            for b in self.bolts: b.update(0) ; b.draw(self.scr)
            ...
            self.draw_t("OPPONENT VIEW", self.f_m, (255,255,255), ox, oy-35, False)
            
            if self.phase == "lobby":
                msg = "READY: [ENTER]" if not self.is_ready else "WAITING..." ; self.draw_t(msg, self.f_m, (255,255,255), WW//2, WH-60)
            ...
            if self.net.game_over:
                self.draw_t("GAME OVER", self.f_h, (255,255,255), WW//2, WH//2 - 40)
                if self.net.winner == self.net.role: self.draw_t("VICTORY!", self.f_m, (0,255,0), WW//2, WH//2 + 30)
                else: self.draw_t("DEFEAT!", self.f_m, (255,0,0), WW//2, WH//2 + 30)
...
if __name__ == "__main__":
    try: asyncio.run(MarioBattlePro().main_loop())
        ...
        with open(f"v81_crash_{PID}.txt", "w") as f: f.write(traceback.format_exc())
