...
# --- V63 - "MARIO BATTLE HYPER MASTER" ---
# Features:
# - HYPER LINE CLEAR: Violent screenshake, massive white-out flash, and shard particles.
# - REAL ANIMATED TURTLES: Scaled to 2.5x with walking bob and better frames.
# - MULTI-TRACK BGM: Rotates through the 3 pro tracks.
# - JEWEL BLOCKS: 4-facet arcade rendering for P1 and P2.
# - SMOOTH PHYSICS: Fixed grounded state and responsive controls.

PID = os.getpid()
LOG_FILE = f"v63_log_{PID}.txt"
...
class SoundManager:
    ...
    def play_bgm(self, track_index=None):
        if not self.enabled: return
        if track_index is None: track_index = random.randint(0, len(self.playlist)-1)
        tk = self.playlist[track_index] ; p = os.path.join('sounds', tk)
...
# --- NETWORK ---
class FirebaseManagerV63:
    def __init__(self, db_url):
        self.db_url = db_url.rstrip('/') ; self.room_id = "battle_v63_hyper"
...
class Particle:
    def __init__(self, x, y, color):
        self.x, self.y = x, y ; self.vx = random.uniform(-6, 6) ; self.vy = random.uniform(-12, -4) ; self.color = color ; self.life = 1.0
    def update(self, dt):
        self.life -= dt * 1.5 ; self.vy += 28 * dt ; self.x += self.vx ; self.y += self.vy ; return self.life > 0
...
class Enemy:
    ...
    def update(self, dt, grid, sm):
        ...
        ix, iy = int(self.x), int(self.y) ; on_solid = False
        if self.y >= 19.0: self.y = 19.0 ; self.vy = 0 ; on_solid = True
        elif iy+1 < 20 and 0 <= ix < 10 and grid[iy+1][ix] is not None: self.y = float(iy) ; self.vy = 0 ; on_solid = True
        if on_solid:
            if self.state == 'falling': self.state = 'walking' ; sm.play('land')
        else: self.state = 'falling'
        if self.state == 'walking':
            self.lifetime -= dt ; nx = int(self.x + self.dir * 0.5)
            ...
            self.x += self.dir * 1.5 * dt ; return self.lifetime <= 0
        return False
...
class TetrisV63:
    def __init__(self):
        pygame.init()
        self.screen = pygame.display.set_mode((WW, WH)) ; pygame.display.set_caption("MARIO BATTLE HYPER MASTER V63")
        self.clock = pygame.time.Clock() ; self.fb = FirebaseManagerV63("https://mario-tetris-game-default-rtdb.firebaseio.com/") ; self.sm = SoundManager()
        try:
        ...
        self.mode = "menu" ; self.load_assets() ; self.reset() ; self.shake = 0
                ...
                if self.mode == "play":
                    ...
                    if self.clearing:
                        ...
                        if self.fb.pending_garbage > 0:
                            self.sm.play('damage') ; self.shake = 10
                        ...
                        if self.fall_dt > 0.8:
                            ...
                            if self.collide(self.pos, self.shape):
                                self.pos[1] -= 1 ; self.sm.play('lock') ; clr = ID_TO_COLOR.get(COLOR_TO_ID.get(None), (100,100,100))
                                ...
                                if self.clearing:
                                    ...
                                    for ri in self.clearing:
                                        for x in range(GW): self.particles.extend([Particle(60+x*BS+16, GRID_Y+ri*BS+16, self.grid[ri][x]) for _ in range(4)])
    ...
    def draw(self):
        sx, sy = 0, 0
        if self.shake > 0: sx, sy = random.randint(-int(self.shake), int(self.shake)), random.randint(-int(self.shake), int(self.shake))
        self.screen.fill((92, 148, 252))
        if self.mode == "menu":
            self.draw_t("MARIO BATTLE HYPER MASTER V63", self.huge, (255,255,255), WW//2, 200) ; self.draw_t("HYPER-ANIMATED TURTLES & LINE EFFECTS", self.font, (255,255,0), WW//2, 350)
        else:
            ...
            for y in range(GH):
                if y in self.clearing and int(time.time()*20)%2==0:
                    pygame.draw.rect(self.screen, (255,255,255), (mx, my+y*BS, GW*BS, BS)) ; continue
                for x in range(GW):
            ...
            for en in self.enemies:
                ...
                if en.state == 'walking': pygame.draw.rect(self.screen, (255,0,0), (mx+en.x*BS, my+en.y*BS-10, BS*(en.lifetime/12.0), 4))
            for p in self.particles:
                s = pygame.Surface((6, 6)) ; s.fill(p.color) ; s.set_alpha(int(p.life*255)) ; self.screen.blit(s, (p.x+sx, p.y+sy))
            ...
            if self.mode == "lobby": self.draw_t("HIT [ENTER] TO READY!", self.font, (255,255,255), WW//2, WH-60) if not self.is_ready else self.draw_t("WAITING FOR BRO...", self.font, (255,255,255), WW//2, WH-60)
...
if __name__ == "__main__":
    try: asyncio.run(TetrisV63().run())
    except:
        with open(f"crash_v63_{PID}.txt", "w") as f: f.write(traceback.format_exc())
        input("CRASH")
