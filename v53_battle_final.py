...
class FirebaseManagerV53:
    def __init__(self, db_url):
        self.db_url = db_url.rstrip('/')
        self.room_id = "battle_v53"
...
class Enemy:
    def __init__(self, x, y, type_id):
        ...
        self.vy = 0 ; self.state = 'falling' # Enemies start falling
        self.lifetime = 10.0 # 10 seconds to stomp or be punished
    ...
    def update(self, dt, grid):
        ...
        if below >= GH or (0 <= ix < GW and grid[below][ix] is not None):
            ...
            # Walk
            if random.random() < 0.05: # occasional move
                 nx = self.x + self.dir
        ...
        if self.state == 'walking':
            self.lifetime -= dt
            return self.lifetime <= 0 # Signal punishment
        return False
...
class TetrisV53:
    def __init__(self):
        ...
        pygame.display.set_caption("MARIO BATTLE PRO V53")
        self.clock = pygame.time.Clock()
        self.fb = FirebaseManagerV53("https://mario-tetris-game-default-rtdb.firebaseio.com/")
        try:
            ...
            for e in pygame.event.get():
                ...
                if e.type == pygame.KEYDOWN:
                    if self.mode == "menu":
                        ...
                        if e.key == pygame.K_LEFT: self.pos[0]-=1 ; 
                        if self.collide(self.pos, self.shape): self.pos[0]+=1
                        if e.key == pygame.K_RIGHT: self.pos[0]+=1 ; 
                        if self.collide(self.pos, self.shape): self.pos[0]-=1
            ...
            if self.mode != "menu":
                ...
                if self.mode == "play":
                    ...
                    for en in self.enemies[:]:
                        ...
                        if punish: # Punish self!
                            self.enemies.remove(en)
                            self.grid.pop(0) ; self.grid.append([(140,140,140) if x!=random.randint(0,9) else None for x in range(GW)])
                        elif en.y > GH + 1: self.enemies.remove(en)
                        elif en.state == 'walking': # ONLY STOMPABLE IF WALKING
                    ...
                    if self.fall_dt > 0.8:
                        ...
                        if self.collide(self.pos, self.shape):
                            self.pos[1] -= 1
                            for bx, by in self.shape[0]:
                                if 0 <= self.pos[1]+by < GH: self.grid[self.pos[1]+by][self.pos[0]+bx] = {k:ID_TO_COLOR[v] for k,v in { 'I':'1','O':'2','T':'3','S':'4','Z':'5','J':'6','L':'7' }.items()}[self.shape[1]]
                            new_g = [r for r in self.grid if any(c is None for c in r)]
                            c = GH - len(new_g)
                            if c > 0:
                                self.score += (c*1000) ; while len(new_g) < GH: new_g.insert(0, [None]*GW)
    ...
    def draw(self):
        ...
        if self.mode == "menu":
            self.draw_t("MARIO BATTLE PRO V53", self.huge, (255,255,255), WW//2, 200)
            self.draw_t("Pick 1 or 2 to Join", self.font, (255,255,0), WW//2, 350)
        else:
            # P1
            ox, oy = 50, 80
            ...
            for y, r in enumerate(self.grid):
                for x, c in enumerate(r):
                    if c: self.draw_b(ox+x*BS, oy+y*BS, c, BS)
            # Piece
            for bx, by in self.shape[0]: 
                c = { 'I':(0,255,255),'O':(255,255,0),'T':(128,0,128),'S':(0,255,0),'Z':(255,0,0),'J':(0,0,255),'L':(255,165,0) }[self.shape[1]]
                self.draw_b(ox+(self.pos[0]+bx)*BS, oy+(self.pos[1]+by)*BS, c, BS, False)
            # Enemies
            for en in self.enemies:
                ...
                if en.state == 'walking': # Lifetime bar
                    pygame.draw.rect(self.screen, (255,0,0), (ox+en.x*BS, oy+en.y*BS-10, BS * (en.lifetime/10.0), 4))

            # HUD
            self.draw_t(f"SCORE:{self.score} STOMPS:{self.stomps}", self.font, (255,255,255), ox, 50, False)
    ...
    def draw_t(self, text, font, color, x, y, c=True):
        if not font: return
        s = font.render(text, True, color) ; r = s.get_rect(center=(x,y)) if c else s.get_rect(topleft=(x,y)) ; self.screen.blit(s, r)

if __name__ == "__main__":
    try: asyncio.run(TetrisV53().run())
    except: 
        with open(f"crash_v53_{PID}.txt", "w") as f: f.write(traceback.format_exc())
        input("CRASH")
