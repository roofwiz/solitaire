...
# --- V85 - "MARIO BATTLE BEST-OF-THREE MASTER" ---
# Features:
# - BEST OF 3 SYSTEM: Tracks "Match Wins" (1/0, 2/1, etc.).
# - ROUND TRANSITIONS: Round ends -> Score updates -> Players ready up for next round.
# - CHAMPIONSHIP ANNOUNCER: Big banner for the final 2nd win.
# - HYPER-STABLE NET: Throttled 3Hz updates + Buffered Attacks.
...
class AudioManager:
    ...
    def _load(self):
        sfx = {'move':'move.wav','rotate':'rotate.wav','lock':'lock.wav','clear':'clear.wav','stomp':'stomp.wav','shell':'impactGeneric_power_001.ogg','hit':'impactBell_heavy_004.ogg','win':'reward_01.wav'}
        for k, f in sfx.items():
            p = get_asset_path(f)
            if os.path.exists(p):
                try:
                    s = pygame.mixer.Sound(p)
                    s.set_volume(0.2 if k=='move' else 0.4)
                    self.sounds[k] = s
                except: pass
    def play(self, k):
        if k in self.sounds:
...
class BattleNetwork:
    ...
    async def join(self, role):
        ...
        if role == "p1":
            d = {"state":"waiting","p1":{"ready":False,"attack_count":0,"grid":"0"*200,"score":0,"stomps":0,"lost":False, "match_wins":0},"p2":{"ready":False,"attack_count":0,"grid":"0"*200,"score":0,"stomps":0,"lost":False, "match_wins":0},"countdown":0}
    ...
    async def report_round_loss(self):
        ...
        # If we are P1, we can also update the opponent's score globally (or they update it when they poll)
        # To be safe, the LOSER sends the update to the winner's win count
        path_opp = f"battles/{self.room_id}/{self.opp_role}"
        opp_data = await asyncio.to_thread(self._api, path_opp)
        new_wins = (opp_data.get('match_wins', 0) if opp_data else 0) + 1
        ...
        if new_wins >= 2:
            ...
            # Reset losing flag for next round
...
class MarioBattleBO3:
    def __init__(self):
        pygame.init() ; self.scr = pygame.display.set_mode((WW, WH)) ; pygame.display.set_caption("MARIO BATTLE BO3 v85")
        ...
        self.phase = "menu" ; self.load_assets() ; self.reset_round() ; self.banners = [] ; self.comms = [] ; self.bolts = [] ; self.shake = 0 ; self.pending_attacks = 0
    ...
    def reset_round(self):
        ...
        self.pce = self.pull() ; self.fall = self.spa_t = self.clr_t = 0 ; self.enemies = [] ; self.is_ready = False ; self.clring = [] ; self.pending_attacks = 0

    def pull(self):
        if not self.bag: self.bag = ['I','O','T','S','Z','J','L'] ; random.shuffle(self.bag)
        t = self.bag.pop() ; d = {'I':[(0,0),(1,0),(2,0),(3,0)], 'O':[(0,0),(1,0),(0,1),(1,1)], 'T':[(1,0),(0,1),(1,1),(2,1)], 'S':[(1,0),(2,0),(0,1),(1,1)], 'Z':[(0,0),(1,0),(1,1),(2,1)], 'J':[(0,0),(0,1),(1,1),(2,1)], 'L':[(2,0),(0,1),(1,1),(2,1)]}
            ...
            if self.phase != "menu":
                ...
                if self.net.state == "countdown":
                    if time.time() - self.net.countdown_time >= 3.0:
                        if self.net.role == "p1": await asyncio.to_thread(self.net._api, f"battles/{self.net.room_id}", "PATCH", {"state":"playing"})
                        self.phase = "play" ; self.reset_round() ; self.banners.append(AchievementBanner("FIGHT!",(0,255,255),self.f_h))
                ...
                if self.phase == "play":
                    ...
                    if self.net.state == "playing":
                        if self.clring:
                            ...
                            # Atomic Sync
                            shadow = [row[:] for row in self.grid] ; sid = {'I':'1','O':'2','T':'3','S':'4','Z':'5','J':'6','L':'7'}[self.pce['id']]
                            for bx, by in self.pce['shape']:
                                gx, gy = self.pos[0]+bx, self.pos[1]+by
                                if 0 <= gx < 10 and 0 <= gy < 20: shadow[gy][gx] = ID_TO_COLOR[sid]
                            s_str = "".join([COLOR_TO_ID.get(c, "0") if c!=(140,140,140) else "G" for row in shadow for c in row])
                            if not self.net.sync_active:
                                atk = self.pending_attacks ; self.pending_attacks = 0
                                asyncio.create_task(self.net.sync_state(s_str, self.score, self.stomps, atk))

                            if self.net.incoming_attack_q > 0:
                                self.audio.play('hit') ; self.shake = 15 ; self.comms.append(CombatText("⚠️ ATTACK!",(255,50,50),320,220,self.f_m))
                            ...
                            self.spa_t += dt
                            if self.spa_t > 8.0: self.spa_t = 0 ; self.enemies.append(ProfessionalEnemy(random.randint(0,9)))
                                ...
                                if en.update(dt, self.grid, self.audio): self.enemies.remove(en)
                                    ...
                                    for bx, by in self.pce['shape']:
                                        if int(en.x+0.5) == self.pos[0]+bx and int(en.y+0.5) == self.pos[1]+by:
                                            ...
                                            self.comms.append(CombatText("HIT! 🚀",(0,255,255),320,260,self.f_m))
    ...
    def draw(self):
        ...
        if self.phase == "menu":
            ...
            self.draw_t("FIRST TO 2 WINS THE SERIES", self.f_m, (255,255,0), WW//2, 350)
            ...
            for fx in self.bolts: fx.draw(self.scr)
            for fx in self.comms: fx.draw(self.scr)
            for fx in self.banners: fx.draw(self.scr)
            # HUD
            ...
            if self.net.state == "match_over":
                ...
                win_txt = "WINNER: MARIO (P1)" if self.net.p1_wins >= 2 else "WINNER: LUIGI (P2)"
                self.draw_t(win_txt, self.f_m, (0,255,0), WW//2, WH//2 + 30)
            elif self.net.state == "waiting":
                msg = "READY UP: [ENTER]" if not self.is_ready else "WAITING..."
...
if __name__ == "__main__":
    try: asyncio.run(MarioBattleBO3().run())
    except: pass
