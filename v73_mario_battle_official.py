...
# --- V73 - "MARIO BATTLE OFFICIAL ULTIMATE PRO (STABLE)" ---
# Features:
# - ZERO SEMICOLONS: Guaranteed stable loading.
# - ROBUST PHYSICS: Column-bottom snapping to prevent falling through blocks.
# - SOFT DROP: Hold [DOWN] for high-speed play.
# - COMBAT HUB: Floating messages for attacks and incoming rows.
# - RED KOOPA AI: Turns at platform edges.

PID = os.getpid()
LOG_FILE = f"v73_log_{PID}.txt"
...
# --- NETWORK ---
class FirebaseManagerV73:
    def __init__(self, db_url):
        self.db_url = db_url.rstrip('/')
        self.room_id = "battle_v73_pro"
        self.role = None
        self.opp_role = None
...
class Enemy:
    ...
    def update(self, dt, grid, sm):
        ...
        # 2. Advanced Collision - Find the floor or top-most block in current column
        ix = int(max(0, min(9, self.x + 0.5)))
        target_ground_y = 19.0 # Default to floor
        
        # Scan from current y down to floor to find first solid block
        # This is more robust than just checking int(y+1)
        start_scan_row = int(max(0, self.y))
        for row in range(start_scan_row, 20):
            if grid[row][ix] is not None:
                target_ground_y = float(row - 1)
                break
        ...
        # 3. Squish Check
        if on_solid:
            # If our current snapped position is inside a block, we were squished
            curr_row = int(self.y + 0.5)
            if 0 <= curr_row < 20 and grid[curr_row][ix] is not None:
                sm.play('damage')
                return True
        ...
        if self.state == 'walking':
            ...
            if next_x < 0 or next_x > 9:
                turn = True
            # Block ahead at the same level
            elif 0 <= nx < 10 and 0 <= int(self.y + 0.5) < 20:
                if grid[int(self.y + 0.5)][nx] is not None:
                    turn = True
            
            # Red turning at edge
            if not turn and self.type_id == 'R' and self.y < 18.5:
                # Check block below future x
                row_below = int(self.y + 1.5)
                if row_below < 20 and grid[row_below][nx] is None:
                    turn = True
...
class TetrisV73:
    def __init__(self):
        ...
        pygame.display.set_caption("MARIO BATTLE OFFICIAL V73")
        self.clock = pygame.time.Clock()
        self.fb = FirebaseManagerV73("https://mario-tetris-game-default-rtdb.firebaseio.com/")
        ...
        while True:
            ...
            if self.mode != "menu":
                ...
                if self.fb.p1_ready and self.fb.p2_ready and self.fb.role == "p1" and self.fb.room_state == "waiting":
                    await asyncio.to_thread(self.fb._req, f"{self.fb.db_url}/battles/{self.room_id}.json", "PATCH", {"state":"countdown", "countdown_start":time.time()})
                # Check Start
                ...
                if self.mode == "play":
                    ...
                    if self.clearing:
                        ...
                        if self.fb.pending_garbage > 0:
                            self.sm.play('damage')
                            self.shake = 12
                            self.messages.append(CombatMessage("INCOMING ATTACK! ⚠️", (255,50,50)))
                        ...
                        if self.fall_dt > fall_speed:
                            ...
                            if self.collide(self.pos, self.shape):
                                ...
                                if self.clearing:
                                    ...
                                    self.clear_t = 0.6
                                    self.shake = 22
                                    self.messages.append(CombatMessage(f"ULTRA STRIKE! +{len(self.clearing)} 🔥", (0,255,255)))
                                    for row_idx in self.clearing:
                                        for x_idx in range(GW):
                                            pixel_col = self.grid[row_idx][x_idx] or (255,255,255)
                                            self.particles.extend([Particle(60+x_idx*BS+16, GRID_Y+row_idx*BS+16, pixel_col) for _ in range(4)])
    ...
    def draw(self):
        ...
        if self.mode == "menu":
            self.draw_t("MARIO BATTLE OFFICIAL V73", self.huge, (255,255,255), WW//2, 200)
            self.draw_t("ZERO-SEMICOLON STABLE ULTIMATE", self.font, (255,255,0), WW//2, 350)
            self.draw_t("Pick Role P1 [1] or P2 [2]", self.font, (255,255,255), WW//2, 400)
        else:
            mx = 60 + sx
            my = GRID_Y + sy
            # Arena
            ...
            for p in self.particles:
                ps = pygame.Surface((6, 6))
                ps.fill(p.color)
                ps.set_alpha(int(p.life*255))
                self.screen.blit(ps, (p.x+sx, p.y+sy))
            
            for m in self.messages:
                use_font = self.huge if "ULTRA" in m.text else self.font
...
if __name__ == "__main__":
    try:
        asyncio.run(TetrisV73().run())
    except:
        with open(f"crash_v73_{PID}.txt", "w") as f:
