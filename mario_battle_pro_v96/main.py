...
# --- V96 - "MARIO BATTLE BO3 ELITE (FOCUS FIX)" ---
# Fixed: 
# - INPUT FOCUS: Added a mandatory "TAP TO FOCUS" cover to ensure browser focus.
# - COORDINATE MAPPING: Buttons now use relative screen percentage to avoid DPI scaling issues.
# - KEYBOARD SUPREMACY: Pressing 1, 2, M, or L is now the priority path for joining.
# - DARK GREEN BACKGROUND: Visual proof of V96 version.
# - ASYNC JOIN: Join process is completely decoupled from the main draw loop.
...
class BattleNetwork:
    ...
    async def join(self, role):
        ...
        print(f"BATTLE: Attempting to join as {role}")
        self.role = role
        if role == "p1":
...
class MarioBattleV96:
    def __init__(self):
        pygame.init()
        self.scr = pygame.display.set_mode((WW, WH))
        pygame.display.set_caption("MARIO BATTLE V96 ELITE")
        ...
        self.btn1 = pygame.Rect(WW//2-220, 380, 200, 100)
        self.btn2 = pygame.Rect(WW//2+20, 380, 200, 100)
        self.reset_round()
        self.last_key = "None"
        self.last_mouse = (0,0)
            ...
            for e in pygame.event.get():
                ...
                if e.type == pygame.KEYDOWN:
                    ...
                    if self.phase == "prestart": self.phase = "menu"
                        ...
                        if e.key == pygame.K_1 or e.key == pygame.K_m: self.join_req("p1")
                        elif e.key == pygame.K_2 or e.key == pygame.K_l: self.join_req("p2")
    ...
    def draw(self):
        # DARK GREEN BACKGROUND = V96 ELITE
        self.scr.fill((10, 40, 10))
        
        if self.phase == "prestart":
            pygame.draw.rect(self.scr, (0,0,0,150), (0,0,WW,WH))
            self.draw_t("INPUT READY", (255,255,255), WW//2, WH//2-50, True, self.f_h)
            self.draw_t("CLICK OR PRESS ANY KEY TO START", (0,255,0), WW//2, WH//2+50, True, self.f_m)
        elif self.phase == "menu":
            self.draw_t("ARENA V96 - SELECT ROLE", (255,255,255), WW//2, 120, True, self.f_m)
            self.draw_t("USE KEYS: [1] FOR MARIO | [2] FOR LUIGI", (255,255,0), WW//2, 220, True, self.f_s)
            
            # Big Role Buttons
            for r,txt,col in [(self.btn1,"[1] MARIO",(255,50,50)), (self.btn2,"[2] LUIGI",(50,255,50))]:
                cl = (200,200,200) if r.collidepoint(pygame.mouse.get_pos()) else (255,255,255)
                pygame.draw.rect(self.scr, cl, r, 0, 15)
                pygame.draw.rect(self.scr, (0,0,0), r, 4, 15)
                self.draw_t(txt, col, r.centerx, r.centery, True, self.f_m)
                
            # Input Debug Info
            self.draw_t(f"LATEST INPUT: {self.last_key}", (200,200,200), WW//2, WH-80, True, self.f_s)
            self.draw_t(f"LATEST MOUSE: {self.last_mouse}", (200,200,200), WW//2, WH-50, True, self.f_s)
        else:
            # Main Battle Arena
            ax, ay = 60, GRID_Y
            ...
            if self.net.status != "Idle": self.draw_t(self.net.status, (255,255,255), WW//2, WH-120, True, self.f_s)
...
if __name__ == "__main__":
    asyncio.run(MarioBattleV96().run())
