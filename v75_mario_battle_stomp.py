...
# --- V75 - "MARIO BATTLE STRIKE (CORR-LOGIC)" ---
# Features:
# - STOMP CORRELATION: Shell projectiles fly towards the opponent on stomp.
# - DYNAMIC COMBAT TEXT: "TURTLE STREAK!" and "STOMP SENT!" messages.
# - LIVE ACTION SYNC: Falling blocks visible in real-time.
# - SNAP PHYSICS: Advanced column scanning for turtle stability.
# - PRO PLAYLIST: Includes Bring The Noise and high-energy tracks.

PID = os.getpid()
LOG_FILE = f"v75_log_{PID}.txt"
...
# --- VISUAL EFFECTS ---
class CombatMessage:
    def __init__(self, text, color, x, y, size='small'):
        self.text = text
        self.color = color
        self.x = x
        self.y = y
...
# --- NETWORK ---
class FirebaseManagerV75:
    def __init__(self, db_url):
        self.db_url = db_url.rstrip('/')
        self.room_id = "battle_v75_strike"
        self.role = None
        self.opp_role = None
...
class TetrisV75:
    def __init__(self):
        ...
        pygame.display.set_caption("MARIO BATTLE STRIKE SYNC V75")
        self.clock = pygame.time.Clock()
        self.fb = FirebaseManagerV75("https://mario-tetris-game-default-rtdb.firebaseio.com/")
        self.sm = SoundManager()
        try:
            f = "assets/PressStart2P-Regular.ttf"
            if os.path.exists(f):
                self.font = pygame.font.Font(f, 13)
                self.huge = pygame.font.Font(f, 35)
                self.giant = pygame.font.Font(f, 55)
            else:
    ...
    def load_assets(self):
        try:
            ...
            self.imgs = {
                ...
                'shell': self.get_sprite(m, 360, 212, 16, 16, 2.0),
            ...
            if self.mode != "menu":
                ...
                if self.mode == "play":
                    ...
                    if self.clearing:
                        ...
                        # LIVE SYNC: Include falling piece
                            ...
                            if en.update(dt, self.grid, self.sm): self.enemies.remove(en)
                                ...
                                for bx, by in self.shape[0]:
                                    if int(en.x + 0.5) == self.pos[0]+bx and int(en.y + 0.5) == self.pos[1]+by:
                                        ...
                                        # Visual Correlation
                                        self.messages.append(CombatMessage("🐢 STOMPED!", (255,255,0), 60+en.x*BS, GRID_Y+en.y*BS))
    ...
    def draw(self):
        ...
        if self.mode == "menu":
            self.draw_t("MARIO BATTLE STRIKE V75", self.giant, (255,255,255), WW//2, 200)
            self.draw_t("STOMP ATTACK CORRELATION", self.huge, (255,255,0), WW//2, 350)
        else:
            ...
            for m in self.messages:
                uf = self.giant if m.size == 'huge' else self.font ; self.draw_t(m.text, uf, m.color, m.x, m.y)
...
if __name__ == "__main__":
    try: asyncio.run(TetrisV75().run())
    except:
        with open(f"crash_v75_{PID}.txt", "w") as f: f.write(traceback.format_exc())
        input("CRASH")
