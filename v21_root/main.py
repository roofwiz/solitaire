...
class Tetris:
    ...
    def handle_line_clear(self, cleared, events=[], line_indices=[]):
        ...
        if self.game_state == 'BATTLE' and self.firebase_manager:

             # Send Garbage: 1 line = 0, 2 = 1, 3 = 2, 4 = 4

             lines_to_send = 0

             if cleared == 2: lines_to_send = 1

             elif cleared == 3: lines_to_send = 2

             elif cleared == 4: lines_to_send = 4

             

             if lines_to_send > 0:

                 self.firebase_manager.send_attack(lines_to_send)

                 self.popups.append(PopupText(WINDOW_WIDTH//2, WINDOW_HEIGHT//2 - 100, f"SENT {lines_to_send} LINES!", C_GREEN))
        ...
        try:
            ...
            if self.game_state == 'BATTLE':
                ...
                if not self.firebase_manager:
                     ...
                     # Escape to Menu if manager died
                ...
                # Check for Trigger (from Intro)

                if getattr(self.intro_scene, 'trigger_battle', False):

                     self.intro_scene.trigger_battle = False

                     # Already in BATTLE state, so likely already initialized

                

                # Poll Network

                self.firebase_manager.poll()

                

                # Check Connection State

                if self.firebase_manager.remote_state == 'waiting':

                     # Show Waiting Screen

                     self.popups.append(PopupText(WINDOW_WIDTH//2, WINDOW_HEIGHT//2, "WAITING FOR OPPONENT...", C_WHITE))

                     # Don't update game logic yet

                     return

                

                # Check for Incoming Garbage / Attacks

                if self.firebase_manager.pending_garbage > 0:

                     print(f"Applying {self.firebase_manager.pending_garbage} garbage lines!")

                     # Add garbage lines from bottom

                     count = self.firebase_manager.pending_garbage
                     ...
                     for _ in range(count):
                         ...
                         new_row[random.randint(0, GRID_WIDTH-1)] = None # One hole

                         self.grid.grid.pop(0) # Remove top

                         self.grid.grid.append(new_row) # Add bottom

                         

                     self.sound_manager.play('damage')

                     self.popups.append(PopupText(WINDOW_WIDTH//2, WINDOW_HEIGHT//2, "ATTACK RECEIVED!", C_RED))



                # Check for Win/Loss (Round End)

                # If WE died (game over logic below handles it, but we need to report it)

                # Actually, standard game over logic sets 'lives' to 0 then state to GAMEOVER
                ...
                if self.lives <= 0:
                     ...
                     self.firebase_manager.report_loss()

                     print("REPORTING LOSS")

                     self.game_state = 'BATTLE_OVER' 

                     self.battle_timer = 5.0 # 5 seconds before next round

                     return

                     

                # Check for Win (Opponent Died)

                # If opponent status is 'dead', we won this round!

                if self.firebase_manager.opponent_status == 'dead':

                      print("OPPONENT DIED - WE WIN ROUND")

                      self.firebase_manager.report_win()

                      self.game_state = 'BATTLE_OVER'

                      self.battle_timer = 5.0

                      return



            if self.game_state == 'BATTLE_OVER':
                ...
                my_wins = self.firebase_manager.wins

                op_wins = self.firebase_manager.opponent_wins

                

                if my_wins >= 2 or op_wins >= 2:

                    if self.battle_timer <= 0:

                         self.game_state = 'INTRO'

                         self.firebase_manager.connected = False # Disconnect
                ...
                if self.battle_timer <= 0:
                    ...
                    self.reset_game() # Resets grid, pieces, etc

                    self.game_state = 'BATTLE'

                    # We should signal "Ready" to DB?

                    # For simplicity, just behave like a new game join logic-wise

                    # But we keep wins/losses
        ...
        try:
            ...
            if self.game_state == 'INTRO':
                ...
                if my_wins >= 2:
                    ...
                    txt = self.font_big.render("VICTORY!", True, C_GREEN)

                    target.blit(txt, txt.get_rect(center=(cx, cy - 50)))

                elif op_wins >= 2:

                    txt = self.font_big.render("DEFEAT", True, C_RED)

                    target.blit(txt, txt.get_rect(center=(cx, cy - 50)))

                else:

                    txt = self.font_med.render(f"ROUND OVER", True, C_WHITE)

                    target.blit(txt, txt.get_rect(center=(cx, cy - 80)))

                    

                    score_txt = self.font_big.render(f"{my_wins} - {op_wins}", True, C_YELLOW)

                    target.blit(score_txt, score_txt.get_rect(center=(cx, cy)))

                    

                    time_txt = self.font_small.render(f"Next Round in {int(self.battle_timer)+1}...", True, C_WHITE)

                    target.blit(time_txt, time_txt.get_rect(center=(cx, cy + 80)))
    ...
    def draw_intro(self):
        ...
        try:
            ...
            battle_btn_rect = pygame.Rect(0, 0, 300, 60); battle_btn_rect.center = (cx, cy + 100)

            self.btn_battle_rect = battle_btn_rect

            pygame.draw.rect(target, (200, 50, 50), battle_btn_rect, border_radius=15)

            pygame.draw.rect(target, C_WHITE, battle_btn_rect, 2, border_radius=15)

            b_surf = self.font_small.render("BATTLE MODE (ONLINE)", True, C_WHITE)

            target.blit(b_surf, b_surf.get_rect(center=battle_btn_rect.center))

            

            # ROOM ID INPUT

            room_id = getattr(self, 'input_room_id', "1234") # Default

            id_surf = self.font_small.render(f"ROOM: {room_id}", True, (200, 200, 200))

            target.blit(id_surf, id_surf.get_rect(center=(cx, cy + 145)))



            esc = self.font_small.render("PRESS ESC TO QUIT", True, C_WHITE)

            target.blit(esc, esc.get_rect(center=(cx, cy + 180)))
            ...
            if mouse_buttons[0]:  # Left click/touch
                ...
                if not hasattr(self, '_intro_click_handled'):
                    ...
                    if self.btn_tetris_rect.collidepoint(mouse_pos):

                        print(f"START SINGLE PLAYER")
                    ...
                    elif self.btn_battle_rect.collidepoint(mouse_pos):

                        print(f"START BATTLE MODE ROOM {room_id}")
