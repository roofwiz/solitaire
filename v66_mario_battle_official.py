...
# --- V66 - "MARIO BATTLE ULTIMATE STRIKE (FIXED)" ---
# Features:
# - FIXED LOADING: Corrected syntax errors from the previous build.
# - OFFICIAL PHYSICS: Turtles snap to blocks and never fall through.
# - FRONT-FACING AI: Koopas face the correct walking direction.
# - RED KOOPA EDGE DETECTION: Red shells turn at the ends of platforms.
# - HYPER-IMPACT: Violent screenshake and colored shard explosions.
# - 4-FACET JEWEL BLOCKS: High-end arcade rendering.

PID = os.getpid()
LOG_FILE = f"v66_log_{PID}.txt"
...
# --- NETWORK ---
class FirebaseManagerV66:
    def __init__(self, db_url):
        self.db_url = db_url.rstrip('/')
        self.room_id = "battle_v66_strike"
        ...
        self.opp_grid_raw = "0" * 200 ; self.pending_garbage = 0 ; self.total_received = 0 ; self.poll_time = 0 
...
class Particle:
    def __init__(self, x, y, color):
        self.x, self.y = x, y
        self.vx, self.vy = random.uniform(-6, 6), random.uniform(-12, -4)
        self.color = color ; self.life = 1.0
    def update(self, dt):
        self.life -= dt * 1.5 ; self.vy += 30 * dt ; self.x += self.vx ; self.y += self.vy
        return self.life > 0

class Enemy:
    def __init__(self, x, type_id='K'):
        self.x, self.y = float(x), -1.0 ; self.type_id = type_id
        self.state = 'falling' ; self.vy = 0 ; self.dir = random.choice([-1, 1])
    ...
    def update(self, dt, grid, sm):
        ...
        if self.y >= 19.0:
            ...
            # Check all rows in current column to find highest block
            for iy in range(20):
        ...
        if on_solid:
            if self.state == 'falling':
                self.state = 'walking' ; sm.play('land')
        else:
            self.state = 'falling'

        if self.state == 'walking':
            self.lifetime -= dt
            nx = int(max(0, min(9, self.x + self.dir * 0.5)))
            ...
            # Boundary
            if self.x + self.dir * 1.5 * dt < 0 or self.x + self.dir * 1.5 * dt > 9:
                self.dir *= -1
            else:
                self.x += self.dir * 1.5 * dt
...
class TetrisV66:
    def __init__(self):
        ...
        pygame.display.set_caption("MARIO BATTLE OFFICIAL V66")
        self.clock = pygame.time.Clock()
        self.fb = FirebaseManagerV66("https://mario-tetris-game-default-rtdb.firebaseio.com/")
                            ...
                            if self.collide(self.pos, self.shape):
                                ...
                                if self.clearing:
                                    ...
                                    for ri in self.clearing:
                                        for x in range(GW):
                                            c_obj = self.grid[ri][x]
                                            c_val = c_obj if c_obj else (100,100,100)
                                            self.particles.extend([Particle(60+x*BS+16, GRID_Y+ri*BS+16, c_val) for _ in range(3)])
    ...
    def draw(self):
        sx, sy = 0, 0
        if self.shake > 0:
            sx, sy = random.randint(-int(self.shake), int(self.shake)), random.randint(-int(self.shake), int(self.shake))
        
        self.screen.fill((92, 148, 252))
        if self.mode == "menu":
            self.draw_t("MARIO BATTLE OFFICIAL V66", self.huge, (255,255,255), WW//2, 200)
            self.draw_t("PRESS [1] OR [2] TO START", self.font, (255,255,0), WW//2, 350)
        else:
            mx, my = 60 + sx, GRID_Y + sy
            pygame.draw.rect(self.screen, (0,0,0,220), (mx-4, my-4, GW*BS+8, GH*BS+8))
            ...
            if not self.clearing:
                for bx, by in self.shape[0]:
                    c_id = self.shape[1]
                    c = {'I':(0,240,240),'O':(240,240,0),'T':(160,0,240),'S':(0,240,0),'Z':(240,0,0),'J':(0,0,240),'L':(240,160,0)}[c_id]
                    self.draw_3d_b(mx+(self.pos[0]+bx)*BS, my+(self.pos[1]+by)*BS, c, BS)
            
            for en in self.enemies:
                img_key = 'koopa' if en.type_id=='K' else 'red_koopa'
                img = self.imgs[img_key][en.anim_frame]
            ...
            for p in self.particles:
                ps = pygame.Surface((6, 6)) ; ps.fill(p.color) ; ps.set_alpha(int(p.life*255))
                self.screen.blit(ps, (p.x+sx, p.y+sy))
            ...
            for i, cid in enumerate(self.fb.opp_grid_raw):
                if cid != '0':
                    self.draw_3d_b(ox+(i%10)*sbs, oy+(i//10)*sbs, ID_TO_COLOR.get(cid, (200,200,200)), sbs)
            ...
            if self.mode == "lobby":
                ...
                self.draw_t(status, self.font, (255,255,255), WW//2, WH-60)
    ...
    def draw_3d_b(self, x, y, col, sz):
        if col == (140,140,140) and 'brick' in self.imgs:
            ...
            return
        lt, dk = [min(255, c+85) for c in col], [max(0, c-85) for c in col]
    ...
    def draw_t(self, t, f, c, x, y, ct=True):
        if f:
            s = f.render(t, True, c)
            r = s.get_rect(center=(x,y)) if ct else s.get_rect(topleft=(x,y))
            self.screen.blit(s, r)

if __name__ == "__main__":
    try: asyncio.run(TetrisV66().run())
    except Exception:
        with open(f"crash_v66_{PID}.txt", "w") as f: f.write(traceback.format_exc())
        input("CRASH")
