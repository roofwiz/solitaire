...
# --- V69 - "MARIO BATTLE OFFICIAL ULTIMATE PRO" ---
# Features:
# - FIXED SYNTAX: Cleaned up code structure for 100% stable loading.
# - SOFT DROP: Holding [DOWN] speeds up falling blocks.
# - ROBUST PHYSICS: Turtles use high-precision collision to stay on stack.
# - COMBAT HUB: "ATTACK SENT" and "RECV GARBAGE" floating messages.
# - PRO PLAYLIST: Rotating tracks from sounds/ folder.

PID = os.getpid()
LOG_FILE = f"v69_log_{PID}.txt"
...
# --- NETWORK ---
class FirebaseManagerV69:
    def __init__(self, db_url):
        self.db_url = db_url.rstrip('/')
        self.room_id = "battle_v69_pro"
        self.role = None
        self.opp_role = None
...
class TetrisV69:
    def __init__(self):
        ...
        pygame.display.set_caption("MARIO BATTLE OFFICIAL V69")
        self.clock = pygame.time.Clock()
        self.fb = FirebaseManagerV69("https://mario-tetris-game-default-rtdb.firebaseio.com/")
                            ...
                            if self.collide(self.pos, self.shape):
                                ...
                                for bx, by in self.shape[0]:
                                    if 0 <= self.pos[1]+by < GH:
                                        self.grid[self.pos[1]+by][self.pos[0]+bx] = c
                                ...
                                if self.clearing:
                                    ...
                                    for ri in self.clearing:
                                        for x in range(GW):
                                            c_val = self.grid[ri][x] if self.grid[ri][x] else (255,255,255)
                                            self.particles.extend([Particle(60+x*BS+16, GRID_Y+ri*BS+16, c_val) for _ in range(4)])
    ...
    def draw(self):
        ...
        if self.mode == "menu":
            self.draw_t("MARIO BATTLE OFFICIAL V69", self.huge, (255,255,255), WW//2, 200)
            self.draw_t("FIXED SYNTAX & SOFT DROP", self.font, (255,255,0), WW//2, 350)
        else:
            ...
            for msg in self.messages:
                f = self.huge if "SENT" in msg.text else self.font
                self.draw_t(msg.text, f, msg.color, msg.x, msg.y)
            
            pimg = self.imgs['mario' if self.fb.role=='p1' else 'luigi'] if 'mario' in self.imgs else None
            if pimg:
                self.screen.blit(pimg, (mx-52, 5))
            ...
            for i, cid in enumerate(self.fb.opp_grid_raw):
                if cid != '0':
                    color = ID_TO_COLOR.get(cid, (200,200,200))
                    self.draw_3d_b(ox+(i%10)*sbs, oy+(i//10)*sbs, color, sbs)
            ...
            if self.fb.room_state == "countdown":
                cd_val = 3 - int(time.time() - self.fb.countdown_start)
                self.draw_t(str(max(1, cd_val)), self.huge, (255,255,255), WW//2, WH//2)
        pygame.display.flip()
    ...
    def draw_3d_b(self, x, y, col, sz):
        if col == (140,140,140) and 'brick' in self.imgs:
            ...
            return
        lt = [min(255, c+85) for c in col]
        dk = [max(0, c-85) for c in col]
...
if __name__ == "__main__":
    try:
        asyncio.run(TetrisV69().run())
    except Exception:
        with open(f"crash_v69_{PID}.txt", "w") as f:
