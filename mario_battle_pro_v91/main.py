...
# --- V91 - "MARIO BATTLE BO3 WEB MASTER" ---
# Fixed: 
# - Removed blocking input() which freezes the browser.
# - Added status indicators for network connectivity.
# - Improved J/L shape precision.
# - High-energy music shuffle.
...
class MarioBattleBO3:
    def __init__(self):
        ...
        pygame.display.set_caption("MARIO BATTLE BO3 v91")
            ...
            if self.phase != "menu":
                ...
                if (self.phase == "lobby" or self.net.state == "countdown") and self.net.state == "playing":
                    ...
                    if time.time() - self.net.countdown_time >= 3.0:
                        if self.net.role == "p1":
                            asyncio.create_task(asyncio.to_thread(self.net._api, f"battles/{self.net.room_id}", "PATCH", {"state":"playing"}))
                        self.phase = "play"
                        self.reset_round()
                        self.banners.append(AchievementBanner("FIGHT!",(0,255,255),self.f_h))
                    ...
                    if self.net.state == "playing":
                        if self.clring:
                            ...
                            if not self.net.sync_active:
                                ...
                                self.pending_attacks = 0
                                asyncio.create_task(self.net.sync_state(s_str, score_v, stomp_v, atk_v))
    ...
    def draw(self):
        ...
        # STATUS BAR
        if self.net.status != "Connected" and self.phase != "menu":
            self.draw_t(f"STATUS: {self.net.status}", self.f_s, (255,255,255), WW//2, WH-20)
        ...
        if self.phase == "menu":
            ...
            if self.net.status != "Idle":
                self.draw_t(self.net.status, self.f_s, (255,255,255), WW//2, 500)
        else:
            ax = 60 + ox
            ay = GRID_Y + oy
            ...
            for en in self.enemies:
                ...
                y_bob = abs(math.sin(time.time()*10))*4
            ...
            if self.net.state == "match_over":
                ...
                if self.is_ready:
                    mmsg = "WAITING..."
                self.draw_t(mmsg, self.f_m, (255,255,255), WW//2, WH - 80)
            elif self.net.state == "countdown":
