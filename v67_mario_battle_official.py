...
# --- V67 - "MARIO BATTLE OFFICIAL ULTIMATE (STABLE PHYSICS)" ---
# Features:
# - PIXEL-PERFECT COLLISION: Improved turtle physics to prevent falling through blocks.
# - ATTACK NOTIFICATIONS: "ATTACK!" and "RECV!" messages on score-events.
# - DYNAMIC PARTICLE EXPLOSIONS: Rich colored shards on line clear.
# - JEWEL BLOCKS: 4-facet arcade rendering.
# - PRO PLAYLIST: Rotating tracks from sounds/ folder.

PID = os.getpid()
LOG_FILE = f"v67_log_{PID}.txt"
...
# --- NETWORK ---
class FirebaseManagerV67:
    def __init__(self, db_url):
        self.db_url = db_url.rstrip('/') ; self.room_id = "battle_v67_ultimate"
...
class FloatingMessage:
    def __init__(self, text, x, y, color):
        self.text, self.x, self.y, self.color = text, x, y, color ; self.life = 1.0
    def update(self, dt):
        self.life -= dt ; self.y -= 30 * dt ; return self.life > 0

class Particle:
    def __init__(self, x, y, color):
        self.x, self.y = x, y ; self.vx, self.vy = random.uniform(-6, 6), random.uniform(-12, -4) ; self.color = color ; self.life = 1.0
    def update(self, dt):
...
class Enemy:
    ...
    def update(self, dt, grid, sm):
        ...
        # Physics
        self.vy += 32 * dt ; self.y += self.vy * dt ; ix = int(max(0, min(9, self.x))) ; on_solid = False
        if self.y >= 19.0: self.y = 19.0 ; self.vy = 0 ; on_solid = True
        else:
            # Better collision probing
            for iy in range(20):
                if grid[iy][ix] is not None:
                    # If turtle's future position is inside/below the top edge of a block
                    if self.y >= float(iy - 0.95):
                        self.y = float(iy - 1) ; self.vy = 0 ; on_solid = True ; break
        if on_solid:
            if self.state == 'falling': self.state = 'walking' ; sm.play('land')
        else: self.state = 'falling'
        if self.state == 'walking':
            self.lifetime -= dt ; nx = int(max(0, min(9, self.x + self.dir * 0.4)))
            if nx != ix and int(self.y) < 20 and grid[int(self.y)][nx] is not None: self.dir *= -1
            elif self.type_id == 'R' and self.y < 19.0:
                iy_below = int(self.y + 1)
                if iy_below < 20 and grid[iy_below][nx] is None: self.dir *= -1
            if self.x + self.dir * 1.5 * dt < 0 or self.x + self.dir * 1.5 * dt > 9: self.dir *= -1
            else: self.x += self.dir * 1.5 * dt ; return self.lifetime <= 0
        return False
...
class TetrisV67:
    def __init__(self):
        pygame.init() ; self.screen = pygame.display.set_mode((WW, WH)) ; pygame.display.set_caption("MARIO BATTLE OFFICIAL V67")
        self.clock = pygame.time.Clock() ; self.fb = FirebaseManagerV67("https://mario-tetris-game-default-rtdb.firebaseio.com/") ; self.sm = SoundManager()
        try:
        ...
        self.mode = "menu" ; self.load_assets() ; self.reset() ; self.shake = 0 ; self.float_msgs = []
    ...
    def reset(self):
        ...
        self.fall_dt = 0 ; self.enemies = [] ; self.score = 0 ; self.stomps = 0 ; self.is_ready = False ; self.spawn_t = 0 ; self.clearing = [] ; self.clear_t = 0 ; self.particles = []
        ...
        while True:
            ...
            if self.mode != "menu":
                ...
                if self.mode == "play":
                    ...
                    self.float_msgs = [m for m in self.float_msgs if m.update(dt)]
                    ...
                    if self.clearing:
                        ...
                        if self.fb.pending_garbage > 0:
                            self.sm.play('damage') ; self.shake = 10 ; self.float_msgs.append(FloatingMessage("RECV GARBAGE! 😱", 60+random.randint(0,100), 200, (255,0,0)))
                            ...
                            if en.update(dt, self.grid, self.sm):
                                ...
                                for bx, by in self.shape[0]:
                                    if int(en.x) == self.pos[0]+bx and int(en.y) == self.pos[1]+by:
                                        ...
                                        self.float_msgs.append(FloatingMessage("STOMP! 🐢🔥", 60+en.x*BS, GRID_Y+en.y*BS, (0,255,0)))
                        ...
                        if self.fall_dt > 0.8:
                            ...
                            if self.collide(self.pos, self.shape):
                                ...
                                if self.clearing:
                                    self.sm.play('clear') ; self.score += (len(self.clearing)*1000) ; self.clear_t = 0.6 ; self.shake = 20
                                    self.float_msgs.append(FloatingMessage(f"ULTRA ATTACK! +{len(self.clearing)} 🔥", 120, 300, (255,255,0)))
                                    for ri in self.clearing:
                                        for x in range(GW): self.particles.extend([Particle(60+x*BS+16, GRID_Y+ri*BS+16, self.grid[ri][x] if self.grid[ri][x] else (255,255,255)) for _ in range(3)])
    ...
    def draw(self):
        ...
        if self.mode == "menu": self.draw_t("MARIO BATTLE OFFICIAL V67", self.huge, (255,255,255), WW//2, 200)
        else:
            mx, my = 60 + sx, GRID_Y + sy ; pygame.draw.rect(self.screen, (0,0,0,220), (mx-4, my-4, GW*BS+8, GH*BS+8)) ; pygame.draw.rect(self.screen, (255,255,255), (mx-4, my-4, GW*BS+8, GH*BS+8), 2)
            ...
            for fm in self.float_msgs: self.draw_t(fm.text, self.font, fm.color, fm.x+sx, fm.y+sy, False)
            ...
            if self.mode == "lobby": self.draw_t("HIT [ENTER] TO READY!", self.font, (255,255,255), WW//2, WH-60)
...
if __name__ == "__main__":
    try: asyncio.run(TetrisV67().run())
    except Exception:
        with open(f"crash_v67_{PID}.txt", "w") as f: f.write(traceback.format_exc())
        input("CRASH")
