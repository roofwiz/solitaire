...
# --- V57 - "MARIO BATTLE ULTIMATE" ---
# Features:
# - SPACE BAR HARD DROP: Drop piece instantly and lock.
# - BGM MUSIC: Background music support (classic Overworld theme).
# - PRO-GRADE PHYSICS: Land on blocks/floor, Red Koopas turn at edges.
# - DUAL HUD: Scores and Stomps for both players.
# - CLASSIC SFX: Stomp, Clear, Lock, Rotate effects.

PID = os.getpid()
LOG_FILE = f"v57_log_{PID}.txt"
...
class SoundManager:
    ...
    def _load_all(self):
        sfx_map = {
            ...
            'stomp':'stomp.wav', 'damage':'stomp.wav', 'move':'move.wav'
        }
    ...
    def play_bgm(self, file_name='music.mp3'):
        ...
        if os.path.exists(path):
            try:
                ...
                log(f"Music Started: {file_name}")
            except Exception as e:
                log(f"BGM Error: {e}")

# --- FIREBASE MANAGER ---
class FirebaseManagerV57:
    def __init__(self, db_url):
        self.db_url = db_url.rstrip('/')
        self.room_id = "battle_v57_ultimate"
...
class TurtleV57:
    ...
    def update(self, dt, grid):
        ...
        if self.y >= 19.0: # Ground
            self.y = 19.0 ; self.vy = 0 ; self.state = 'walking'
        elif below < 20 and 0 <= ix < 10 and grid[below][ix] is not None:
            self.y = float(below - 1) ; self.vy = 0 ; self.state = 'walking'
...
class TetrisV57:
    def __init__(self):
        ...
        pygame.display.set_caption("MARIO BATTLE ULTIMATE V57")
        self.clock = pygame.time.Clock()
        self.fb = FirebaseManagerV57("https://mario-tetris-game-default-rtdb.firebaseio.com/")
        ...
        while True:
                    ...
                    if self.mode == "menu":
                        ...
                        if e.key == pygame.K_SPACE: # HARD DROP
                            while not self.collide([self.pos[0], self.pos[1]+1], self.shape):
                                self.pos[1] += 1
                            self.fall_dt = 100 # Force lock immediately
            ...
            if self.mode != "menu":
                ...
                if self.mode == "play":
                    ...
                    self.spawn_t += dt
                    if self.spawn_t > 9.0:
                        self.spawn_t = 0 ; self.enemies.append(TurtleV57(random.randint(0,9), random.choice(['K','R'])))
    ...
    def draw(self):
        ...
        if self.mode == "menu":
            self.draw_t("MARIO BATTLE ULTIMATE V57", self.huge, (255,255,255), WW//2, 200)
            self.draw_t("Press [1] or [2] to Battle!", self.font, (255,255,0), WW//2, 350)
            self.draw_t("SPACE: Hard Drop | Music Included!", self.font, (255,255,255), WW//2, 400)
        else:
            mx, my = 60, 100
            ...
            if self.mode == "lobby":
                msg = "READY UP!" if not self.is_ready else "WAITING FOR BRO..."
...
if __name__ == "__main__":
    try: asyncio.run(TetrisV57().run())
    except:
        with open(f"crash_v57_{PID}.txt", "w") as f: f.write(traceback.format_exc())
        input("CRASH")
