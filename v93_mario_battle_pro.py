...
# --- V93 - "MARIO BATTLE BO3 CHAMPIONSHIP (ULTRA-STABLE)" ---
# Fixed: 
# - VISUAL PROOF: Changed background to DARK PURPLE to verify you are on the latest version.
# - CLICK TO START: Large overlay to ensure Media Engagement & Focus.
# - RECT BUTTONS: Real clickable buttons for Mario/Luigi.
# - NO BLOCKING: Removed all blocking network calls.
...
class MarioBattleV93:
    def __init__(self):
        pygame.init()
        self.scr = pygame.display.set_mode((WW, WH))
        pygame.display.set_caption("MARIO BATTLE V93 CHAMPIONSHIP")
        ...
        self.phase = "prestart" # prestart -> menu -> lobby -> play
        self.reset_round()
        self.btn1 = pygame.Rect(WW//2-220, 380, 200, 70)
        self.btn2 = pygame.Rect(WW//2+20, 380, 200, 70)
        self.load_assets()
            ...
            for e in pygame.event.get():
                ...
                if e.type == pygame.MOUSEBUTTONDOWN:
                    if self.phase == "prestart": self.phase = "menu"
                        ...
                        if self.btn1.collidepoint(mv): asyncio.create_task(self.net.join("p1")); self.phase = "lobby"
                        elif self.btn2.collidepoint(mv): asyncio.create_task(self.net.join("p2")); self.phase = "lobby"
                if e.type == pygame.KEYDOWN and self.phase == "play":
            ...
            if self.phase != "prestart" and self.phase != "menu":
                ...
                if self.phase == "play":
                    ...
                    if self.net.state == "playing":
                        ...
                        cid = {'I':'1','O':'2','T':'3','S':'4','Z':'5','J':'6','L':'7'}[self.pce['id']]
                        for bx, by in self.pos[1]+by, self.pos[0]+bx for bx, by in self.pce['shape']:
    ...
    async def sync_logic(self, s_str):
        try:
            ...
            await asyncio.to_thread(self.net._req, path, "PATCH", {"grid": s_str, "match_wins": self.net.p1_wins if self.net.role=="p1" else self.net.p2_wins})
            self.net.last_sync = time.time()
        finally: self.net.sync_active = False

    def draw(self):
        # VISUAL PROOF: PURPLE BACKGROUND
        self.scr.fill((40, 20, 60)) 
        
        if self.phase == "prestart":
            self.draw_t("V93 CHAMPIONSHIP READY",(255,255,255),WW//2, WH//2 - 50, True, self.f_h)
            self.draw_t("CLICK ANYWHERE TO ENTER",(0,255,255),WW//2, WH//2 + 50, True, self.f_m)
        elif self.phase == "menu":
            self.draw_t("CHOOSE YOUR FIGHTER",(255,255,255),WW//2, 200, True, self.f_h)
            for r,txt,col in [(self.btn1,"MARIO", (255,50,50)), (self.btn2,"LUIGI", (50,255,50))]:
                pygame.draw.rect(self.scr, (255,255,255), r, 0, 10)
...
if __name__ == "__main__":
    asyncio.run(MarioBattleV93().run())
