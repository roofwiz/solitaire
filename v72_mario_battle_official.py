...
# --- V72 - "MARIO BATTLE OFFICIAL ULTIMATE (STRIKE V2)" ---
# Features:
# - TWO-COLUMN COLLISION: Turtles check left/right edges to stay on stack.
# - SQUISH LOGIC: Turtles die if occupied by a block (prevents falling through).
# - ROBUST SOFT DROP: Holding [DOWN] is more responsive.
# - SYNCED COUNTDOWN: Reliable network transition from lobby to play.
# - DYNAMIC COMBAT TEXT: Large notifications for attacks and incoming rows.

PID = os.getpid()
LOG_FILE = f"v72_log_{PID}.txt"
...
# --- NETWORK ---
class FirebaseManagerV72:
    def __init__(self, db_url):
        self.db_url = db_url.rstrip('/')
        self.room_id = "battle_v72_ultimate"
        self.role = None
        self.opp_role = None
...
class Enemy:
    ...
    def update(self, dt, grid, sm):
        ...
        # 2. Collision (Check columns floor(x) and ceil(x))
        col1 = int(max(0, min(9, self.x)))
        col2 = int(max(0, min(9, self.x + 0.9)))
        ...
        if self.y >= 19.0:
            ...
            # Check for solid ground at exact row
            iy = int(self.y + 1)
            if 0 <= iy < 20:
                if grid[iy][col1] is not None or grid[iy][col2] is not None:
                    # SNAPPING logic
                    if self.y >= float(iy - 1.0):
                        self.y = float(iy - 1)
                        self.vy = 0
                        landed = True
        ...
        # 3. Squish Detection (If inside a block, die)
        ix = int(self.x + 0.5)
        iyy = int(self.y + 0.5)
        if 0 <= ix < 10 and 0 <= iyy < 20:
            if grid[iyy][ix] is not None:
                sm.play('damage')
                return True # Signal death/removal

        # 4. Horizontal Movement
        if self.state == 'walking':
            self.lifetime -= dt
            # Face correct direction logic: dir=1 is RIGHT, dir=-1 is LEFT
            move_step = self.dir * 1.5 * dt
            next_x = self.x + move_step
            ...
            if next_x < 0 or next_x > 9:
                turn = True
            elif nx != col1 and iyy < 20 and grid[iyy][nx] is not None:
                turn = True
...
class TetrisV72:
    def __init__(self):
        ...
        pygame.display.set_caption("MARIO BATTLE ULTIMATE V72")
        self.clock = pygame.time.Clock()
        self.fb = FirebaseManagerV72("https://mario-tetris-game-default-rtdb.firebaseio.com/")
            ...
            if self.mode != "menu":
                ...
                if self.mode == "play":
                    ...
                    if self.clearing:
                        self.clear_t -= dt
                        if self.clear_t <= 0:
                            new_g = [r for r_idx, r in enumerate(self.grid) if r_idx not in self.clearing]
                        ...
                        if self.fb.pending_garbage > 0:
                            ...
                            self.messages.append(CombatMessage("GARBAGE INCOMING! ⚠️", (255,100,100)))
                        ...
                        for en in self.enemies[:]:
                            if en.update(dt, self.grid, self.sm):
                                ...
                                # Death was signaled (either stomp or time or squish)
                                # If it wasn't a stomp (handled below), it was a squish or lifetime
                                if en.state != 'walking': self.sm.play('damage')
                        ...
                        fall_speed = 0.8
                        if pygame.key.get_pressed()[pygame.K_DOWN]: fall_speed = 0.05
                            
                        self.fall_dt += dt
                        if self.fall_dt > fall_speed:
                            self.fall_dt = 0 ; self.pos[1] += 1
                            if self.collide(self.pos, self.shape):
                                self.pos[1] -= 1 ; self.sm.play('lock') ; c_type = self.shape[1]
                                ...
                                if self.clearing:
                                    ...
                                    for row_idx in self.clearing:
                                        for x_idx in range(GW):
                                            ...
                                            self.particles.extend([Particle(60+x_idx*BS+16, GRID_Y+row_idx*BS+16, pc) for _ in range(3)])
    ...
    def draw(self):
        ...
        if self.mode == "menu":
            self.draw_t("MARIO BATTLE ULTIMATE V72", self.huge, (255,255,255), WW//2, 200)
            self.draw_t("TWO-COLUMN COLLISION & SQUISH LOGIC", self.font, (255,255,0), WW//2, 350)
            self.draw_t("READY FOR PRO STRIKES", self.font, (255,255,255), WW//2, 400)
        else:
            ...
            for y in range(GH):
                if y in self.clearing and int(time.time()*20)%2==0: pygame.draw.rect(self.screen, (255,255,255), (mx, my+y*BS, GW*BS, BS))
                    ...
                    for x in range(GW):
                        color = self.grid[y][x] ; if color: self.draw_3d_b(mx+x*BS, my+y*BS, color, BS)
            if not self.clearing and self.mode == "play":
                ccol = {'I':(0,240,240),'O':(240,240,0),'T':(160,0,240),'S':(0,240,0),'Z':(240,0,0),'J':(0,0,240),'L':(240,160,0)}[self.shape[1]]
                for bx, by in self.shape[0]: self.draw_3d_b(mx+(self.pos[0]+bx)*BS, my+(self.pos[1]+by)*BS, ccol, BS)
            # Enemies
            for en in self.enemies:
            ...
            for m in self.messages:
                txt_font = self.huge if "ULTRA" in m.text else self.font ; self.draw_t(m.text, txt_font, m.color, m.x, m.y)
            ...
            self.draw_t(f"OPP: {self.fb.opp_score}", self.font, (255,255,255), ox, oy-30, False)
            if self.mode == "lobby":
                status = "ENTER TO READY!" if not self.is_ready else "READY!" ; color = (255,255,255) if not self.is_ready else (100,255,100)
                self.draw_t(status, self.font, color, WW//2, WH-60)
            if self.fb.room_state == "countdown":
                sec = 3 - int(time.time() - self.fb.countdown_start)
                if sec >= 0: self.draw_t(str(max(1, sec)), self.huge, (255,255,255), WW//2, WH//2)
        pygame.display.flip()
...
if __name__ == "__main__":
    try: asyncio.run(TetrisV72().run())
    except:
        with open(f"crash_v72_{PID}.txt", "w") as f: f.write(traceback.format_exc())
        input("CRASH")
