...
# --- V65 - "MARIO BATTLE OFFICIAL (PRO PHYSICS)" ---
# Features:
# - SNAPPING PHYSICS: Turtles land on the highest blocks and never fall through.
# - DIRECTIONAL AI: Correct facing direction (no walking backward).
# - RED KOOPA EDGE AI: Red turtles turn at platform edges.
# - HYPER LINE CLEAR: Screenshake and colored shard particles.
# - 4-FACET JEWEL BLOCKS: Premium arcade rendering.
# - PRO PLAYLIST: 3 requested high-energy tracks.

PID = os.getpid()
LOG_FILE = f"v65_log_{PID}.txt"
...
# --- NETWORK ---
class FirebaseManagerV65:
    def __init__(self, db_url):
        self.db_url = db_url.rstrip('/') ; self.room_id = "battle_v65_pro"
...
class Enemy:
    ...
    def update(self, dt, grid, sm):
        ...
        if self.y >= 19.0:
            ...
            # Check every block in this column below the turtle
            # Iterate through rows to find the highest block
            for iy in range(20):
                if grid[iy][ix] is not None:
                    # If turtle's feet (y+1) enter the brick, snap to top
                    if self.y >= float(iy - 1) and self.y - self.vy * dt < float(iy - 1):
        ...
        if self.state == 'walking':
            self.lifetime -= dt ; next_x = self.x + self.dir * 1.5 * dt
            ...
            # Red turning at edge
                ...
                if iy_below < 20 and grid[iy_below][nx] is None and self.y < 19.0: self.dir *= -1
...
class TetrisV65:
    def __init__(self):
        pygame.init()
        self.screen = pygame.display.set_mode((WW, WH)) ; pygame.display.set_caption("MARIO BATTLE OFFICIAL V65")
        self.clock = pygame.time.Clock() ; self.fb = FirebaseManagerV65("https://mario-tetris-game-default-rtdb.firebaseio.com/") ; self.sm = SoundManager()
        try:
            ...
            if self.mode != "menu":
                ...
                if self.mode == "play":
                    ...
                    if self.clearing:
                        ...
                        self.spawn_t += dt
                        if self.spawn_t > 8.0: self.spawn_t = 0 ; self.enemies.append(Enemy(random.randint(0,9), random.choice(['K','R'])))
                            ...
                            if self.collide(self.pos, self.shape):
                                ...
                                if self.clearing:
                                    ...
                                    for ri in self.clearing:
                                        for x in range(GW): self.particles.extend([Particle(60+x*BS+16, GRID_Y+ri*BS+16, self.grid[ri][x]) for _ in range(3)])
    ...
    def draw(self):
        ...
        if self.mode == "menu": self.draw_t("MARIO BATTLE OFFICIAL V65", self.huge, (255,255,255), WW//2, 200)
        else:
            ...
            for y in range(GH):
                if y in self.clearing and int(time.time()*20)%2==0: pygame.draw.rect(self.screen, (255,255,255), (mx, my+y*BS, GW*BS, BS)) ; continue
                for x in range(GW):
            ...
            for en in self.enemies:
                ...
                # FACING LOGIC: Flip if walking RIGHT (Original faces left)
            ...
            if self.mode == "lobby": self.draw_t("READY?", self.font, (255,255,255), WW//2, WH-60)
...
if __name__ == "__main__":
    try: asyncio.run(TetrisV65().run())
    except:
        with open(f"crash_v65_{PID}.txt", "w") as f: f.write(traceback.format_exc())
        input("CRASH")
