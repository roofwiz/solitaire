...
# --- V28 LOGGING ---
DEBUG_FILE = "v28_final_log.txt"
def log(msg):
    with open(DEBUG_FILE, "a") as f:
        f.write(f"[{time.strftime('%H:%M:%S')}] {msg}\n")
    print(f"[V28] {msg}")

with open(DEBUG_FILE, "w") as f: f.write("--- Mario Tetris V28 Launch ---\n")
...
class FirebaseManager:
    ...
    async def _fetch(self, url, method="GET", data=None):
        try:
            ...
            log(f"Fetch {method} {url}")
            if method == "GET": r = requests.get(url, timeout=5)
            elif method == "PUT": r = requests.put(url, json=data, timeout=5)
            elif method == "PATCH": r = requests.patch(url, json=data, timeout=5)
            return r.json()
...
# Standardized Shapes for all versions
...
class TetrisV28:
    def __init__(self):
        ...
        pygame.display.set_caption("Mario Tetris V28 - STABLE LOCAL BATTLE")
            ...
            if self.fb.connected:
                ...
                if self.fb.pending_garbage > 0:
                    ...
                    for _ in range(self.fb.pending_garbage):
                        self.grid.pop(0)
                        row = [random.choice(list(T_SHAPES.values()))[1] for _ in range(GRID_WIDTH)]
                ...
                if self.fall_time > 0.6:
                    ...
                    if self.check_collision(self.current_pos, self.current_shape):
                        ...
                        while len(new_grid) < GRID_HEIGHT: new_grid.insert(0, [None]*GRID_WIDTH)
                        self.grid = new_grid
    ...
    def draw(self):
        ...
        if not self.fb.connected:
            self.screen.blit(self.font.render(">> PRESS ENTER TO BATTLE <<", True, (0, 255, 0)), (WINDOW_WIDTH//2-200, WINDOW_HEIGHT//2))
        pygame.display.flip()

if __name__ == "__main__":
    try:
        game = TetrisV28(); asyncio.run(game.run())
    except Exception as e:
        log(traceback.format_exc())
        with open("v28_CRASH_REPORT.txt", "w") as f: f.write(traceback.format_exc())
        input("CRASH DETECTED! Check v28_CRASH_REPORT.txt. Press Enter...")
