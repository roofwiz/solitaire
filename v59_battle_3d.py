...
# --- V59 - "MARIO BATTLE ULTIMATE 3D" ---
# Features:
# - PRO 3D BLOCKS: Real bevel/shadow highlights for a professional look.
# - LINE CLEAR ANIMATION: Smooth white-flash effect when clearing rows.
# - ADJUSTED LAYOUT: Lifted the grid so the floor is perfectly visible.
# - SPACE BAR HARD DROP: Insta-slam pieces down.
# - BACKGROUND MUSIC: Loops 'intro_theme.mp3'.

PID = os.getpid()
LOG_FILE = f"v59_log_{PID}.txt"
            ...
            for p in paths:
                if os.path.exists(p):
                    try:
                        ...
                        self.sounds[name].set_volume(0.5 if name != 'move' else 0.3)
                        break
                    except: pass
...
# --- FIREBASE MANAGER ---
class FirebaseManagerV59:
    def __init__(self, db_url):
        self.db_url = db_url.rstrip('/')
        self.room_id = "battle_v59_ultimate"
...
class TurtleV59:
    def __init__(self, x, type_id='K'):
        ...
        self.lifetime = 12.0 ; self.speed = 1.0 ; self.anim_frame = 0 ; self.anim_timer = 0

    def update(self, dt, grid, sound_man):
        self.anim_timer += dt
        ...
        if on_solid:
            if self.state == 'falling': self.state = 'walking'; sound_man.play('land')
...
GW, GH = 10, 20
BS = 32
# Y-LIFT: Positioning the grid so the floor is clearly visible
GRID_Y = 50 
...
class TetrisV59:
    def __init__(self):
        ...
        pygame.display.set_caption("MARIO BATTLE ULTIMATE 3D V59")
        self.clock = pygame.time.Clock()
        self.fb = FirebaseManagerV59("https://mario-tetris-game-default-rtdb.firebaseio.com/")
    ...
    def reset(self):
        ...
        self.clearing_lines = [] ; self.clear_anim_t = 0
        ...
        while True:
            ...
            for e in pygame.event.get():
                ...
                if e.type == pygame.KEYDOWN:
                    if self.mode == "menu":
                    ...
                    elif self.mode == "play" and not self.clearing_lines:
                        m = False
                        ...
                        if e.key == pygame.K_SPACE: 
                            ...
                            while not self.collide([self.pos[0], self.pos[1]+1], self.shape):
                                self.pos[1] += 1 ; dropped = True
                            if dropped: self.sm.play('lock') ; self.fall_dt = 10.0 
            ...
            if self.mode != "menu":
                ...
                if self.mode == "play":
                    # Line Clear Animation Logic
                    if self.clearing_lines:
                        self.clear_anim_t -= dt
                        if self.clear_anim_t <= 0:
                            new_g = [r for i, r in enumerate(self.grid) if i not in self.clearing_lines]
                            count = GH - len(new_g)
                            while len(new_g) < GH: new_g.insert(0, [None]*GW)
                            self.grid = new_g ; self.clearing_lines = []
                            self.pos = [GW//2-1, 0] ; self.shape = self.spawn()
                            if self.collide(self.pos, self.shape): self.reset()
                    else:
                        sync_grid = "".join([COLOR_TO_ID.get(c, "0") if c!=(140,140,140) else "G" for r in self.grid for c in r])
                        asyncio.create_task(self.fb.sync(sync_grid, self.score, self.stomps))
                        ...
                        self.spawn_t += dt
                        if self.spawn_t > 9.0:
                            self.spawn_t = 0 ; self.enemies.append(TurtleV59(random.randint(0,9), random.choice(['K','R'])))
                        ...
                        if self.fall_dt > 0.8:
                            ...
                            if self.collide(self.pos, self.shape):
                                ...
                                # Line Check
                                self.clearing_lines = [i for i, r in enumerate(self.grid) if all(x is not None for x in r)]
                                if self.clearing_lines:
                                    self.sm.play('clear') ; self.score += (len(self.clearing_lines) * 1000)
                                    self.clear_anim_t = 0.4
                                    asyncio.create_task(self.fb.attack(len(self.clearing_lines)))
                                else:
                                    self.pos = [GW//2-1, 0] ; self.shape = self.spawn()
                                    if self.collide(self.pos, self.shape): self.reset()
    ...
    def draw(self):
        ...
        if self.mode == "menu":
            self.draw_t("MARIO BATTLE ULTIMATE 3D V59", self.huge, (255,255,255), WW//2, 200)
            self.draw_t("PRO-GRADE 3D BLOCKS & SOUND", self.font, (255,255,0), WW//2, 350)
            self.draw_t("Press [1] or [2] to Battle!", self.font, (255,255,255), WW//2, 400)
        else:
            mx, my = 60, GRID_Y
            # Playfield Outline
            pygame.draw.rect(self.screen, (0,0,0,180), (mx, my, GW*BS, GH*BS))
            pygame.draw.rect(self.screen, (255,255,255), (mx, my, GW*BS, GH*BS), 2)
            ...
            # Grid Blocks
            for y in range(GH):
                if y in self.clearing_lines and (int(time.time()*15)%2==0): continue # Flashing
                for x in range(GW):
                    c = self.grid[y][x]
            ...
            if p: self.screen.blit(p, (mx-50, 5))
            self.draw_t(f"SCORE: {self.score}", self.font, (255,255,255), mx, 30, False)
            self.draw_t(f"STOMPS: {self.stomps}", self.font, (100,255,100), mx + 200, 30, False)

            # Opponent (Right)
            ox, oy = 620, GRID_Y+100 ; sbs = 26
    ...
    def draw_3d_b(self, x, y, color, size):
        ...
        # Highlight (Top/Left)
        h = [min(255, c + 70) for c in color]
        pygame.draw.line(self.screen, h, (x, y), (x+size, y), 2)
        pygame.draw.line(self.screen, h, (x, y), (x, y+size), 2)
        # Shadow (Bottom/Right)
        s = [max(0, c - 70) for c in color]
        pygame.draw.line(self.screen, s, (x, y+size-1), (x+size, y+size-1), 2)
        pygame.draw.line(self.screen, s, (x+size-1, y), (x+size-1, y+size), 2)
...
if __name__ == "__main__":
    try: asyncio.run(TetrisV59().run())
    except Exception:
        with open(f"crash_v59_{PID}.txt", "w") as f: f.write(traceback.format_exc())
        input("CRASH")
