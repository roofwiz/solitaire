...
class SlotMachine:
    def __init__(self, sprite_manager=None, sound_manager=None):
        ...
        # Betting
        self.bet_amount = 10 # Coins per line
        self.lines_bet = 1 # 1, 2, or 3 (mapped to lines)
        ...
        # Symbols & Reels
        self.symbols = ['mario', 'luigi', 'mushroom', 'star', 'coin', 'shell', 'spiny', 'flower', 'flyer', 'jackpot', 'wild']
        # Normal Weights (used when betting coins)
        self.normal_weights = [15, 15, 15, 10, 20, 10, 5, 8, 8, 2, 5]
        # BONUS Weights - Much better odds for the player during free spins!
        # Higher chances for jackpot, wild, star, flower - lower for common items
        self.bonus_weights = [8, 8, 12, 18, 15, 8, 3, 15, 12, 8, 12]
        self.weights = self.normal_weights  # Default to normal
    ...
    def start_spin(self):
        ...
        self.stop_timers = [0.5, 0.8, 1.1] # Reel stop delays
        ...
        if self.state == 'BONUS_INTRO':
            ...
            if self.msg_timer <= 0:
                ...
                if self.spins_remaining <= 0:
                    ...
                    self.show_total_timer = 4.0  # Show for 4 seconds
                    if self.session_winnings > 0:
                        self.msg = f"★ TOTAL WON: +{int(self.session_winnings)} ★"
                    else:
    ...
    def _calc_line_win(self, syms):
        ...
        if all(s == eff or s == 'wild' for s in syms):
             # WIN!
             base = 0
             if eff == 'jackpot': base = 500
             elif eff == 'wild': base = 1000 # 3 Wilds
             elif eff == 'star': base = 100
             elif eff == 'mario': base = 50
             elif eff == 'luigi': base = 40
             elif eff == 'coin': base = 20
             else: base = 10
