...
# --- V64 - "MARIO BATTLE HYPER MASTER RELOADED" ---
# Features:
# - FIXED SYNTAX: Resolved "not loading" errors.
# - HYPER LINE CLEAR: Violent screenshake and shard particles.
# - REAL ANIMATED TURTLES: 2.0x scale with walking bob.
# - 4-FACET JEWEL BLOCKS: High-end arcade rendering.
# - PRO PLAYLIST: Rotating soundtrack of the 3 requested mp3s.

PID = os.getpid()
LOG_FILE = f"v64_log_{PID}.txt"
...
# --- NETWORK ---
class FirebaseManagerV64:
    def __init__(self, db_url):
        self.db_url = db_url.rstrip('/')
        self.room_id = "battle_v64_hyper"
...
class Particle:
    ...
    def update(self, dt):
        self.life -= dt * 1.5 ; self.vy += 28 * dt ; self.x += self.vx ; self.y += self.vy
        return self.life > 0
...
class TetrisV64:
    def __init__(self):
        ...
        pygame.display.set_caption("MARIO BATTLE HYPER MASTER V64")
        self.clock = pygame.time.Clock()
        self.fb = FirebaseManagerV64("https://mario-tetris-game-default-rtdb.firebaseio.com/")
                            ...
                            if self.collide(self.pos, self.shape):
                                ...
                                if self.clearing:
                                    ...
                                    for ri in self.clearing:
                                        for x in range(GW): self.particles.extend([Particle(60+x*BS+16, GRID_Y+ri*BS+BS//2, self.grid[ri][x]) for _ in range(4)])
    ...
    def draw(self):
        ...
        if self.mode == "menu":
            self.draw_t("MARIO BATTLE HYPER MASTER V64", self.huge, (255,255,255), WW//2, 200)
            self.draw_t("STABLE & HYPER-ANIMATED", self.font, (255,255,0), WW//2, 350)
            ...
            for p in self.particles:
                psurf = pygame.Surface((6, 6)) ; psurf.fill(p.color) ; psurf.set_alpha(int(p.life*255))
...
if __name__ == "__main__":
    try: asyncio.run(TetrisV64().run())
    except:
        with open(f"crash_v64_{PID}.txt", "w") as f: f.write(traceback.format_exc())
        input("CRASH")
