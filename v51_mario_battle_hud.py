...
class FirebaseManagerV51:
    def __init__(self, db_url):
        self.db_url = db_url.rstrip('/')
        self.room_id = "battle_v51"
        self.role = None 
        self.opp_role = None
...
class ShellAnim:
    ...
    def get_pos(self):
        # Quadratic curve for fun
        px = self.x + (self.tx - self.x) * self.t
        py = self.y + (self.ty - self.y) * self.t - 100 * math.sin(math.pi * self.t)
...
class TetrisV51:
    def __init__(self):
        ...
        pygame.display.set_caption("MARIO BATTLE HUD V51")
        self.clock = pygame.time.Clock()
        self.fb = FirebaseManagerV51("https://mario-tetris-game-default-rtdb.firebaseio.com/")
    ...
    def reset_game(self):
        ...
        self.fall_dt = 0; self.spawn_timer = 3.0
            ...
            if self.mode != "menu":
                ...
                if self.mode == "play":
                    ...
                    if self.spawn_timer > 6.0:
                        self.spawn_timer = 0
                        self.enemies.append(Enemy(random.randint(0, 9), -1, random.choice(['K','S'])))
    ...
    def draw(self):
        ...
        if self.mode == "menu":
            self.draw_t("MARIO BATTLE PRO V51", self.huge_font, (255,255,255), WW//2, 200)
            self.draw_t("1 = Mario, 2 = Luigi, Space = Window", self.font, (255,255,0), WW//2, 350)
        else:
            # My Area
            mx, my = 60, 80
            ...
            self.draw_t(f"STOMP: {self.stomps}", self.font, (0,255,0), mx + 160, 50, False)
            p = self.imgs['mario' if self.fb.role=='p1' else 'luigi']
            self.screen.blit(p, (mx-50, 48))
            
            # Opponent Area
            ox, oy = 580, 80 ; sbs = 25
            self.draw_g(ox, oy, [[(ID_TO_COLOR[c] if c!='0' else None) for c in self.fb.opp_grid_raw[i*10:(i+1)*10]] for i in range(20)], None, None, sbs)
            # HUD Right
    ...
    def draw_g(self, ox, oy, grid, pos, shape, size):
        pygame.draw.rect(self.screen, (0,0,0,150), (ox, oy, GW*size, GH*size))
...
if __name__ == "__main__":
    try: asyncio.run(TetrisV51().run())
    except: 
        with open(f"crash_v51_{PID}.txt", "w") as f: f.write(traceback.format_exc())
        input("CRASH")
