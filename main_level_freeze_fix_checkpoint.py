...
class Tetris:
        ...
        try:
            ...
            if self.game_state == 'WORLD_CLEAR':
             ...
             if self.transition_timer >= 5.0: # 5 Seconds Results
                 ...
                 # Proceed to Bonus
                 try:
                     self.slot_trigger_reason = 'LEVEL_WIN'
                     self.game_state = 'SLOT_MACHINE'
                     self.slot_machine.trigger(spins=getattr(self, 'kills_this_level', 0))
    ...
    def _draw_actual_game(self):
        try:
            ...
            if getattr(self, 'show_level_intro', False):
             ...
             txt2 = self.font_med.render("STOMP TURTLES = COINS", True, C_GREEN)
             txt3 = self.font_med.render("STOMP 3 AT ONCE = SPIN BONUS!", True, C_NEON_PINK)
             
             s = 2
             self.game_surface.blit(txt1, txt1.get_rect(center=(cx+s, cy+s)))
             self.game_surface.blit(txt1, txt1.get_rect(center=(cx, cy)))
             self.game_surface.blit(txt2, txt2.get_rect(center=(cx, cy + 60)))
             self.game_surface.blit(txt3, txt3.get_rect(center=(cx, cy + 100)))
    ...
    def draw_world_clear(self):
        # Stats Screen - Use Overlay, NOT fill
        ...
        l3 = self.font_med.render(f"STOMPS: {self.kills_this_level}", True, C_GREEN)
        
        # Enemy Unlocks Logic (Flavor)
        unlocks = ["RED KOOPAS", "PARATROOPAS", "SPINYS", "HAMMER BROS", "BOWSER?"]
        e_idx = (self.world - 1) * 4 + (self.level_in_world - 1)
        new_enemy = unlocks[e_idx % len(unlocks)]
        
        l4 = self.font_med.render(f"UNLOCKED: {new_enemy}!", True, (255, 215, 0))
        
        self.screen.blit(l1, l1.get_rect(center=(cx, cy - 100)))
        self.screen.blit(l2, l2.get_rect(center=(cx, cy)))
        self.screen.blit(l3, l3.get_rect(center=(cx, cy + 50)))
        self.screen.blit(l4, l4.get_rect(center=(cx, cy + 100)))
        ...
        m_frame = None
        if hasattr(self, 'sprite_manager'):
             m_frame = self.sprite_manager.get_sprite('mario', 'run', index=int(anim_time)%3, scale_factor=3.0)
        ...
        if m_frame:
            ...
            # Fallback if sprite missing (Debug Red Box)
            mx = (self.transition_timer / 5.0) * (WINDOW_WIDTH + 100) - 50
            pygame.draw.rect(self.screen, C_RED, (mx, WINDOW_HEIGHT - 100, 50, 50))
