...
class Tetris:
    def __init__(self):
        ...
        pygame.display.set_caption("Mario Tetris V27 - LOCAL BATTLE")
        ...
        while True:
            ...
            for e in pygame.event.get():
                ...
                if e.type == pygame.KEYDOWN:
                    if e.key == pygame.K_RETURN and not self.firebase.connected and not self.connecting:
    ...
    async def connect_network(self):
        res = await self.firebase.join_room("battle_v27")
        if res: self.role = res
        else: self.role = "ROOM FULL"
        self.connecting = False
    ...
    def draw(self):
        ...
        if not self.firebase.connected:
            self.screen.blit(self.font.render(">> PRESS ENTER TO START BATTLE <<", True, (0, 255, 0)), (WINDOW_WIDTH//2-200, WINDOW_HEIGHT//2))
        
        pygame.display.flip()

if __name__ == "__main__":
    try:
        game = Tetris(); asyncio.run(game.run())
