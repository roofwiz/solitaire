...
# --- V84 - "MARIO BATTLE STABLE (MUSIC & SFX MASTER)" ---
# Technical Specs:
# - PRO PLAYLIST: Shuffles between all high-energy tracks (Bring The Noise, Ten in 2010, etc.)
# - DYNAMIC VOLUME: Balanced BGM and SFX for a premium competitive feel.
# - ZERO-SHORTHAND: Every statement on its own line for 100% stability.
# - BUFFERED COMBAT: Attacks are recorded locally to prevent network freezes.
...
class AchievementBanner:
    ...
    def draw(self, screen):
        ...
        screen.blit(surf, surf.get_rect(center=(WW // 2, WH // 4 + self.y_offset)))
...
class AudioManager:
    ...
    def start_bgm(self):
        try:
            # Pick a random high-energy track
            track = random.choice(self.playlist)
            p = get_asset_path(track)
            if os.path.exists(p):
                pygame.mixer.music.load(p)
                pygame.mixer.music.set_volume(0.3) # Increased for impact
...
class BattleNetwork:
    ...
    def _api(self, ep, method="GET", body=None):
        try:
            ...
            if body is not None:
                ...
                with urllib.request.urlopen(req, data=data, timeout=4) as r:
    ...
    async def join(self, role):
        ...
        if role == "p1":
            d = {
                "state": "waiting",
                "p1": {"ready": False, "attack_count": 0, "grid": "0"*200, "score": 0, "stomps": 0, "lost": False},
                "p2": {"ready": False, "attack_count": 0, "grid": "0"*200, "score": 0, "stomps": 0, "lost": False},
                "countdown": 0,
                "winner": ""
            }
        ...
        if n - self.last_poll_time > POLL_INTERVAL:
            ...
            if data:
                ...
                if rem_q > self.synced_received_total:
                    new_lines = rem_q - self.synced_received_total
                    self.incoming_attack_q += new_lines
    ...
    async def sync_state(self, grid, score, stomps, attacks_to_add):
        ...
        try:
            ...
            await asyncio.to_thread(self._api, path, "PATCH", new_payload)
...
# --- GAME ENGINE ---
class TetrisBattleV84:
    def __init__(self):
        pygame.init()
        self.scr = pygame.display.set_mode((WW, WH))
        pygame.display.set_caption("MARIO BATTLE STABLE v84")
    ...
    def load_assets(self):
        try:
            m_path = get_asset_path("marioallsprite.png")
            b_path = get_asset_path("blocks.png")
            m = pygame.image.load(m_path).convert_alpha()
            b = pygame.image.load(b_path).convert_alpha()
            self.img = {
        ...
        while True:
                ...
                if e.type == pygame.KEYDOWN:
                    if self.phase == "menu":
                        ...
                        if e.key == pygame.K_RETURN:
                            ...
                            await asyncio.to_thread(self.net._api, path, "PATCH", {"ready": self.is_ready})
            ...
            if self.phase != "menu":
                ...
                if self.net.p1_ready and self.net.p2_ready and self.net.role == "p1" and self.net.state == "waiting":
                    u = f"battles/{self.net.room_id}"
                    await asyncio.to_thread(self.net._api, u, "PATCH", {"state": "countdown", "countdown": time.time()})
                if self.net.state == "countdown" and self.phase == "lobby":
                    if time.time() - self.net.countdown_time >= 3.0:
                        self.phase = "play"
                        self.reset()
                        self.banners.append(AchievementBanner("START!", (0, 255, 255), self.f_h))
                    ...
                    if not self.net.game_over:
                        if self.clring:
                            ...
                            if self.clr_t <= 0:
                                ...
                                self.pce = self.pull()
                                if self.collide(self.pos, self.pce['shape']):
                                    await self.net.declare_lost()
                        else:
                            # Atomic sync only
                            shadow = [row[:] for row in self.grid]
                            c_id_char = {'I': '1', 'O': '2', 'T': '3', 'S': '4', 'Z': '5', 'J': '6', 'L': '7'}[self.pce['id']]
                            c_col = ID_TO_COLOR[c_id_char]
                            ...
                            if not self.net.sync_active:
                                p_atk = self.pending_attacks
                                self.pending_attacks = 0
                                asyncio.create_task(self.net.sync_state(s_str, self.score, self.stomps, p_atk))

                            if self.net.incoming_attack_q > 0:
                                self.audio.play('hit')
                                self.shake = 15
                                self.comms.append(CombatText("⚠️ BLOCK IN!", (255, 50, 50), 320, 220, self.f_m))
    ...
    def draw(self):
        ...
        if self.phase == "menu":
            ...
            self.draw_t("MUSIC POLISH v84", self.f_m, (255, 255, 0), WW // 2, 350)
            ...
            if not self.clring and self.phase == "play" and not self.net.game_over:
                c_id_char = {'I': '1', 'O': '2', 'T': '3', 'S': '4', 'Z':'5','J':'6','L':'7'}[self.pce['id']]
                cl = ID_TO_COLOR[c_id_char]
                for bx, by in self.pce['shape']:
                    self.draw_3db(ax + (self.pos[0] + bx) * BS, ay + (self.pos[1] + by) * BS, cl, BS)
            ...
            # FX
            for b_fx in self.bolts:
                b_fx.draw(self.scr)
            for c_fx in self.comms:
                c_fx.draw(self.scr)
            for b_fx in self.banners:
                b_fx.draw(self.scr)
            ...
            if self.net.game_over:
                self.draw_t("MATCH OVER", self.f_h, (255, 255, 255), WW // 2, WH // 2 - 40)
                if self.net.winner == self.net.role:
                    self.draw_t("VICTORY!", self.f_m, (0, 255, 0), WW // 2, WH // 2 + 30)
                else:
                    self.draw_t("DEFEAT!", self.f_m, (255, 0, 0), WW // 2, WH // 2 + 30)
            elif self.phase == "lobby":
                msg = "READY: [ENTER]"
                if self.is_ready:
                    msg = "WAITING..."
                ...
                if sec >= 0:
                    self.draw_t(str(max(1, sec)), self.f_h, (255, 255, 255), WW // 2, WH // 2)
...
if __name__ == "__main__":
    try:
        asyncio.run(TetrisBattleV84().run())
    except Exception:
        pass
