...
# --- V58 - "MARIO BATTLE PERFECT" ---
# Features:
# - SPACE BAR HARD DROP: Insta-slam pieces down.
# - SOUNDS FROM PROJECT DIR: Uses the actual high-quality sound files from /sounds.
# - BACKGROUND MUSIC: Loops 'intro_theme.mp3' for that official feel.
# - PRO-GRADE PHYSICS: Turtles land on blocks/floor, Red Koopas are smart.
# - DUAL HUD: Scores and Stomps for both players.

PID = os.getpid()
LOG_FILE = f"v58_log_{PID}.txt"
...
class SoundManager:
    ...
    def _load_all(self):
        # We'll look in root and in /sounds
        sfx_map = {
            'rotate': 'rotate.wav', 
            'lock': 'lock.wav', 
            'clear': 'clear.wav', 
            'stomp': 'stomp.wav', 
            'move': 'move.wav',
            'damage': 'impactBell_heavy_004.ogg', # Punish sound
            ...
            for p in paths:
                if os.path.exists(p):
                    try:
                        ...
                        # Calibrate volumes
                        vol = 0.5
                        if name == 'move': vol = 0.3
                        if name == 'rotate': vol = 0.4
                        if name == 'stomp': vol = 0.7
                        self.sounds[name].set_volume(vol)
                        break
                    except: pass
        
        # Fallbacks
        if 'damage' not in self.sounds and 'stomp' in self.sounds: self.sounds['damage'] = self.sounds['stomp']
    ...
    def play_bgm(self):
        ...
        # intro_theme.mp3 is usually the classic theme in this project
        ...
        if os.path.exists(path):
            try:
                ...
                log("BGM Started: intro_theme.mp3")
            except Exception as e:
                log(f"BGM Error: {e}")
        else:
            # Try underground as fallback
            path = os.path.join('sounds', '02. undergroundtheme.mp3')
            if os.path.exists(path):
                try:
                    pygame.mixer.music.load(path)
                    pygame.mixer.music.set_volume(0.25)
...
# --- FIREBASE MANAGER ---
class FirebaseManagerV58:
    def __init__(self, db_url):
        self.db_url = db_url.rstrip('/')
        self.room_id = "battle_v58_perfect"
...
class TurtleV58:
    def __init__(self, x, type_id='K'):
        ...
        self.lifetime = 12.0 ; self.speed = 1.0
    ...
    def update(self, dt, grid, sound_man):
        ...
        if self.state == 'walking':
            ...
            if nx < 0 or nx >= 10 or (int(self.y) < 20 and grid[int(self.y)][nx] is not None):
                self.dir *= -1
            elif self.type_id == 'R': # Red stays on platforms
                nb = int(self.y + 1)
...
class TetrisV58:
    def __init__(self):
        ...
        pygame.display.set_caption("MARIO BATTLE PERFECT V58")
        self.clock = pygame.time.Clock()
        self.fb = FirebaseManagerV58("https://mario-tetris-game-default-rtdb.firebaseio.com/")
        ...
        while True:
                ...
                if e.type == pygame.KEYDOWN:
                    if self.mode == "menu":
                        ...
                        if e.key == pygame.K_SPACE: # HARD DROP
                            ...
                            if dropped: self.sm.play('lock') ; self.fall_dt = 10.0 # Force lock now
            ...
            if self.mode != "menu":
                ...
                if self.mode == "play":
                    ...
                    self.spawn_t += dt
                    if self.spawn_t > 9.0:
                        self.spawn_t = 0 ; self.enemies.append(TurtleV58(random.randint(0,9), random.choice(['K','R'])))
    ...
    def draw(self):
        ...
        if self.mode == "menu":
            self.draw_t("MARIO BATTLE PERFECT V58", self.huge, (255,255,255), WW//2, 200)
            self.draw_t("1 = Mario, 2 = Luigi, Space = P2 Window", self.font, (255,255,0), WW//2, 350)
            self.draw_t("Music & Classic SFX Included!", self.font, (255,255,255), WW//2, 400)
        else:
            mx, my = 60, 100
            ...
            self.draw_t(f"STOMPS: {self.stomps}", self.font, (100,255,100), mx + 200, 70, False)
...
if __name__ == "__main__":
    try: asyncio.run(TetrisV58().run())
    except Exception:
        with open(f"crash_v58_{PID}.txt", "w") as f: f.write(traceback.format_exc())
        input("CRASH")
