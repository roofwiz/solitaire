...
# --- V90 - "MARIO BATTLE BO3 STABILITY MASTER (PHYSICS FIX)" ---
# Fixed: 
# - AttributeError: 'stomps' was missing in reset_round.
# - Phase Transition: Ensured P2 joins the game when state becomes "playing".
# - Removed all semicolons for absolute stability.
...
class MarioBattleBO3:
    def __init__(self):
        ...
        pygame.display.set_caption("MARIO BATTLE BO3 v90")
    ...
    async def run(self):
        while True:
            ...
            if self.phase != "menu":
                ...
                # TRANSITION TO PLAY PHASE (Gated by current phase)
                if self.phase == "lobby":
                    ...
                    if self.net.state == "playing":
                        if self.clring:
                            ...
                            if not self.net.sync_active:
                                ...
                                stomps_val = self.stomps
                                asyncio.create_task(self.net.sync_state(s_str, score_val, stomps_val, atk_val))
...
if __name__ == "__main__":
    try:
        ...
        with open("v90_crash.txt", "w") as f:
