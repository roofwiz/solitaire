...
with open(LOG_FILE, "w") as f: f.write("--- Mario Tetris V25 Startup ---\n")
...
class Tetris:
    def __init__(self):
        ...
        pygame.display.set_caption("Mario Tetris V25 - FIXED LOCAL BATTLE")
    ...
    async def run(self):
        ...
        while True:
            ...
            if network_task is None and not self.firebase.connected:
                log("Starting network join...")
                network_task = asyncio.create_task(self.firebase.join_room("battle_v25"))

            # FIX: Check if network_task is a valid task object before calling .done()
            if network_task and hasattr(network_task, 'done') and network_task.done():
                try:
                    res = network_task.result()
                    if res:
                        self.role = res
                        log(f"Connection Success: {self.role}")
                    else:
                        self.role = "ROOM FULL"
                        log("Connection Failed: Room Full")
                    network_task = "COMPLETED" # String is safe now
                except Exception as e:
            ...
            if self.fall_time > 0.8:
                ...
                if self.check_collision(self.current_pos, self.current_shape):
                    ...
                    if self.check_collision(self.current_pos, self.current_shape): self.grid = [[None]*GRID_WIDTH for _ in range(GRID_HEIGHT)]
